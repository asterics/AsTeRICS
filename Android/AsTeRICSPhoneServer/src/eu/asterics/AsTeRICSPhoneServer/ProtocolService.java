...
/**
 * 
 * This class implements the communication protocol for connection between AsTeRICS and the phone.
 *  
...
public class ProtocolService {
	...
	public ProtocolService(Socket socket){
		...
		try{
			...
			Log.e("ProtocolService", "Stream error: "+e.getMessage());
			ready=false;
		}
	}
	...
	public boolean checkHeaderAvailable(){
		if(!ready){
			Log.d("ProtocolService", "Not ready");
			return false;
		}
	...
	public String decodeCallNumber (byte[] data){
		...
		try{
			...
			Log.e("ProtocolService", "NullPointerException: "+e.getMessage());
			return null;
		}catch(IndexOutOfBoundsException e1){
			Log.e("ProtocolService", "IndexOutOfBoundsException: "+e1.getMessage());
			return null;
		}catch(UnsupportedEncodingException e2){
			Log.e("ProtocolService", "UnsupportedEncodingException: "+e2.getMessage());
			return null;
		}
	}
	...
	public String decodeSMSNumber(byte[] data){
		...
		if(offset+1>=data.length){
			Log.e("ProtocolService", "package error");
			return null;
		}
	...
	public short decodeResult(byte[] data){
		try {
			...
			//e.printStackTrace();
			Log.e("ProtocolService", "ResultDecode error ");
			return 0;
		}
	}
	...
	public boolean sendResultPackage(int result){
		byte header[] = prepareHeader(Command.Result);
		
		try {
			Log.d("ProtocolService", "Result: " + result);
			outputStream.write(header);
			short DataSize=2;
			outputStream.writeShort(DataSize);
	...
	public boolean sendNewSMSPackage(String phoneNumber, String messageContent){
		...
		try {
			...
			// TODO Auto-generated catch block
			//e1.printStackTrace();
			Log.e("ProtocolService", "SMS send: UnsupportedEncodingException");
			return false;
		}
		...
		try {
			outputStream.write(header);
			short DataSize=(short)(phone.length+message.length+1);
			Log.d("ProtocolService", " d: " + phone.length + " + " + message.length + " +1 =" + DataSize);
			outputStream.writeShort(DataSize);
			byte[]phoneSize= new byte[1];
			phoneSize[0]=(byte)phone.length;
			outputStream.write(phoneSize);
			...
			Log.e("ProtocolService", "SMS send: IOException");
			return false;
		}
		
		Log.d("ProtocolService","SMS send: OK");
		return true;
	...
	public byte[] getData(int size){
		if(size==0){
			...
			if(readBytes<size){
				Log.e("ProtocolService", "No all data downloaded");
			}
	...
	public HeaderData decodeHeader()
	{
		...
		try{
			...
			if(readBytes< headerBegin-offset){
				...
				Log.e("ProtocolService", "is offset: " + offset);
			...
			if((header[0]==firstByte)&&(header[1]==secondByte)){
				...
				if(command==Command.None){
					Log.e("ProtocolService", "commend nont found ");
				}
				
				if(checkCommandData(command)==false && dataSize>0){
					Log.e("ProtocolService", "data inconsist ");
				}
				
				Log.d("ProtocolService", "command: " + command);
				//return dataSize;
				return new HeaderData(command,dataSize);
			}else{
				boolean found=false;
				Log.e("ProtocolService", "header begin not recognized " + header[0] + " " + header[1]);
