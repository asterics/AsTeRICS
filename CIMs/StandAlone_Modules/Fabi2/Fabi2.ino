...
/* 
   ...
   (sent via serial interface, use spaces between parameters and Enter (<cr>, ASCII-code 0x0d) to finish a command)
          ...
          AT SAVE <name>    save modes and settings to next free eeprom slot under given name (e.g. AT SAVE mouse1)
          AT LOAD <name>    load modes and settings from eeprom slot (e.g. AT LOAD mouse1 -> loads profile named "mouse1")
