...
const char HID_CIM_FEATURELIST[]=
{
   ...
   0x04,0x00,  // mouse x/y absolute pos   data: 4 bytes xx/yy
   ...
   0x20,0x00,  // joystick update          data: 19 bytes b/b/pov/x1/y2/x2/y2/uuuu/tr/ci/cr/sq/l1/r1/l2/r2
} ;
...
uint8_t process_ARE_frame(uint8_t status_code)
{
	...
	LEDs_ToggleLEDs(LED6);  // indicate correct frame
	      ...
	      switch (command)   {  // process requested command
			...
			case CMD_REQUEST_WRITE_FEATURE:  //  write feature to CIM
					...
					switch (ARE_frame.cim_feature) {  // which feature address ?
						...
						case HID_CIM_FEATURE_MOUSEABSPOS_XY: 
						  if (data_size==4)  {
						     if (free_in_InBuf(&input_absmouse) > 5) {
						  		insert_InBuf(&input_absmouse,(uint8_t)ARE_frame.cim_feature);
								for(i=0;i<data_size;i++)
  					  			   insert_InBuf(&input_absmouse,ARE_frame.data[i]);
  								new_insert=&input_absmouse;
							 }
						  } else status_code |= CIM_ERROR_INVALID_FEATURE;
						  break;
						...
						case HID_CIM_FEATURE_JOYSTICKUPDATE:
						  if (data_size==19)  {
  						     ...
  						     if (free_in_InBuf(&input_joystick) > 20) {
...
//		LEDs_ToggleLEDs(LED7);  // indicate wrong feature
//	}
	...
	if (ack_needed) {
		CIM_frame.reply_code|=(((uint16_t)status_code)<<8);
	    reply_Acknowledge();
	}
...
void reply_FeatureList(void)
{
    ...
    if (free_bytes < OUTBUF_WARNING) {
		 ...
		 LEDs_TurnOnLEDs(LED7);  // indicate frame loss
    }
	if (free_bytes > 29) {
