...
//############################################################################
{
    ...
    if (Buffer[510] == 0x55 && Buffer[511] == 0xAA)
    {
       //usb_send_str("VBR Signatur found!\n"); 
    }
    else
    {
       //usb_send_str("VBR Signatur not found!\r\n"); 
...
//############################################################################
{
	...
	if(mmc_write_sector (block,buffer))
	{
		usb_send_str("Error writing file");
	}
	else
	{
		usb_send_str("Written to file...OK\n");
	}
	return;
}
