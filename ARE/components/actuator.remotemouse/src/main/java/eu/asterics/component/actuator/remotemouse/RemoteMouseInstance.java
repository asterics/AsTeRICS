...
/**
 *   RemoteMouseInstance interfaces with HID actuator USB-dongle 
 *   to emulate a mouse on a remote PC via USB HID
 *   
 *  
...
public class RemoteMouseInstance extends AbstractRuntimeComponentInstance
{
    ...
    private boolean centerXState = true; 
    private boolean centerYState = true; 
   ...
   synchronized private void sendXY(int sendX, int sendY)
   {
		...
		if (port != null)
		{
			//    		AstericsErrorHandling.instance.reportInfo(this, String.format("sending packet: diffX: %x, diffY: %x, byteArray: %x, %x, %x, %x", diffX, diffY, b[0], b[1], b[2], b[3]));
			CIMPortManager.getInstance().sendPacket(port, b, HID_FEATURE_MOUSE_XY_REL, CIMProtocolPacket.COMMAND_REQUEST_WRITE_FEATURE, false);
		}
   }
   ...
   /**
    * process the coordinate input for the HId mouse
    * if necessary, large movements (>127) are translated in to several steps.
    * if selected, the mouse action is initiated only if both (x and y) coordinates
    * have been received.
    */
   synchronized private void processRelativeInput(double dx, double dy)
    {
    	...
    	if (System.currentTimeMillis() - lastUpdate > REFRESH_INTERVAL)
    	{
    		/*
    		switch (centerXState) { 
    			case 1: diffX=-5000;diffY=-5000;
    					input1Arrived=true;input2Arrived=true;
    					centerXState++;
    				 break;
    		...
    		if (text.startsWith("@MOUSE:")) {  			
				try {		
					...
					else if (command.equalsIgnoreCase("action"))
					{
						...
						if (actionType.equalsIgnoreCase("enable")) enableMouseAction = true;
						else if (actionType.equalsIgnoreCase("disable")) enableMouseAction = false;
						else if (actionType.equalsIgnoreCase("toggle")) enableMouseAction = !enableMouseAction;
					}
