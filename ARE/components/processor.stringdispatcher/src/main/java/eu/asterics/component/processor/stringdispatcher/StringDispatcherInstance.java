...
public class StringDispatcherInstance extends AbstractRuntimeComponentInstance {
    ...
    /**
     * Sends the string value from the previous slot. If the current current
     * slot is the first, tries to send value from the last slot.
     */
    ...
    /**
     * Sends the string value from the next slot. If the current current slot is
     * the last, tries to send value from the first slot.
     */
    void sendNext() {
        int index = currentIndex + 1;
        for (int i = 0; i < NUMBER_OF_SLOTS; i++) {
