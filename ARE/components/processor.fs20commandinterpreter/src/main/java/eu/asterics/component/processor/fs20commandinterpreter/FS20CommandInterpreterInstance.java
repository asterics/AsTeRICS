...
/**
 * 
 * This processor receive a FS20 input string, containing housecode, sender
 * address and FS20 command. If the housecode and the sender address fits to the
 * setted properties, an event regarding to the command will be fired. This
 * functionality is also included in the FS20Receiver plugin, but because of the
 * fact, that the FS20Receiver must be a singelton, this plugin is needed to
 * work with several FS20 inputs.
 * 
 * 
 * @author Roland Ossmann [ro@ki-i.at] Date: 16.05.2012 Time: 10:06
 */
public class FS20CommandInterpreterInstance extends AbstractRuntimeComponentInstance {
    ...
    private final IRuntimeInputPort ipCommand = new DefaultRuntimeInputPort() {
        ...
        public void receiveData(byte[] data) {
            ...
            if (values.length != 3) {
                AstericsErrorHandling.instance.reportInfo(new FS20CommandInterpreterInstance(),
                        "Parameter mismatch for action string " + receivedCommand
                                + "! Format is: hc_addr_cmd! example: 11111111_1111_18 for toggle");
                return;
            }
            try {
                ...
                AstericsErrorHandling.instance.reportInfo(new FS20CommandInterpreterInstance(),
                        "Data mismatch in action string " + receivedCommand
                                + "! Numbers could not be converted. Format is: hc_addr_cmd! example: 11111111_1111_18 for toggle");
            }
        }
    };
