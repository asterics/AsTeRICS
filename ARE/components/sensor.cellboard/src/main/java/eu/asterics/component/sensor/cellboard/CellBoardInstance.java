...
/**
 * 
 * Implement GUI Cell Board component. The component can display up to 36 cells
 * with text or/and picture.
 * 
...
public class CellBoardInstance extends AbstractRuntimeComponentInstance {
    ...
    // final IRuntimeEventTriggererPort etpSelectedCell = new
    ...
    public Object setRuntimePropertyValue(String propertyName, Object newValue) {
        ...
        if (PROP_CAPTION.equalsIgnoreCase(propertyName)) {
            ...
            // also set the xmlFile variable to the new value, so the next time
            // the cellboard can be loaded from the file
        ...
        try {
            if (key.equals(PROP_KEYBOARD_FILE)) {
                ...
                for (File file : files) {
                    ...
                    // //.getPath().substring(file.getPath().indexOf("set")));
            ...
            AstericsErrorHandling.instance.getLogger()
                    .warning("Could not search for keyboard files: " + e.getMessage());
        }
        return res;
    }
    ...
    public void loadXmlFile() {
        if (xmlFile == null || "".equals(xmlFile)) {
            // AstericsErrorHandling.instance.getLogger().fine("CellBoard: no
            // xmlFile for keyboard set");
            return;
        }
        ...
        try {
            ...
            /*
             * int hheight = handler.getHeight(); if (hheight >= 0) space.height
             * = hheight; else space.height = getAvailableSpace().height; int
             * hwidth = handler.getWidth(); if (hwidth >= 0) space.width =
             * hwidth; else space.width = getAvailableSpace().width;
             */
            paintCells.run();
     ...
     * 
     * @param fileName
     *            name (and path) of the xml file to be created
     */
     ...
     *            index of the cell
     * @param path
     *            path of the preview sound file
     */
