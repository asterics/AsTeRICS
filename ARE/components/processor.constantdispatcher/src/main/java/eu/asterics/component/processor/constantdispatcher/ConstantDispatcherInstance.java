...
/**
 *Implements the Constant Dispatcher plugin, which stores and sends double values
 * 
...
public class ConstantDispatcherInstance  extends AbstractRuntimeComponentInstance
{
  ...
  {
    super.start();
    //System.out.println("Sending slot "+propAutosendSlot+"="+propSlotArray[propAutosendSlot-1]);
  ...
  /**
   * Sends the double value from the previous slot. 
   * If the current current slot is the first,
   * sends the double value from the slot defined by the number property.
   */  
  ...
  /**
   * Sends the double value from the next slot. 
   * If the current current slot is the slot defined by the number property,
   * sends the double value the first slot.
   */  
