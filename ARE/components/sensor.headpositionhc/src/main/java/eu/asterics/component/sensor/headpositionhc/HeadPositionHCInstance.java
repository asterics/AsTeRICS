 ...
 * 
 * 
 * @author Cornelia Salomon [cornelia.salomon@technikum-wien.at] Date: Time:
 */
public class HeadPositionHCInstance extends AbstractRuntimeComponentInstance implements GrabbedImageListener {
    ...
    public void start() {
        ...
        try {
            FrameGrabber grabber = SharedFrameGrabber.instance.getFrameGrabber(Integer.toString(this.propCameraID), 640,
                    480);
    ...
    private void initialize() {
        ...
        HaarCascadeDetection.setHAAR_DIR(propPathForHaarCascade);
        counter = propCounterResettingROI + 1; // if no feature is found counter
                                               // ++ => until counterReset then
                                               // a new ROI is build
    ...
    public void imageGrabbed(IplImage snapIm) {
        try {
            ...
            // detect frontal face
            if (roiRect == null || counter > propCounterResettingROI) // get new
                                                                      // ROI as
                                                                      // starting
                                                                      // position
            {
                ...
                paintComponent(g, snapIm); // draws the Rectangles and Choices
                                           // in the current image and displays
                                           // the image
                greyImage.release();
                return;
            }
    ...
    private void CreateChoices() {
        ...
        for (int i = 0; i < (numChoices * 2); i += 2) {
            ...
            name = "Auswahl " + (i + 2);
            choices[i + 1] = new Choices(name, width * 2, height * (i / 2), width, height, propCounterToSendSelectEvent,
                    this);
        }
    }
    ...
    private IplImage convertFrame(IplImage img) {
        ...
        IplImage grayImg = AbstractIplImage.create(img.width(), img.height(), IPL_DEPTH_8U, 1);
    ...
    private void paintROIs(Graphics g, IplImage snapImage) {
        if (roiRect != null) {
            cvRectangle(snapImage, cvPoint(roiRect.x(), roiRect.y()),
                    cvPoint(roiRect.x() + roiRect.width(), roiRect.y() + roiRect.height()), AbstractCvScalar.RED, 1,
                    CV_AA, 0);
        }
        if (leftearRect.x() != 0) {
            int x = leftearRect.x() + roiRect.x();
            int y = leftearRect.y() + roiRect.y();
            cvRectangle(snapImage, cvPoint(x, y), cvPoint(x + leftearRect.width(), y + leftearRect.height()),
                    AbstractCvScalar.YELLOW, 1, CV_AA, 0);
        }
        if (mouthRect.x() != 0) {
            if (leftearRect.x() != 0) {
                int x = mouthRect.x() + roiRect.x() + (roiRect.width() / 2);
                int y = mouthRect.y() + roiRect.y();
                cvRectangle(snapImage, cvPoint(x, y), cvPoint(x + mouthRect.width(), y + mouthRect.height()),
                        AbstractCvScalar.MAGENTA, 1, CV_AA, 0);
            } else {
        ...
        if (rightearRect.x() != 0) {
            ...
            cvRectangle(snapImage, cvPoint(x, y), cvPoint(x + rightearRect.width(), y + rightearRect.height()),
                    AbstractCvScalar.WHITE, 1, CV_AA, 0);
        }
    }
}