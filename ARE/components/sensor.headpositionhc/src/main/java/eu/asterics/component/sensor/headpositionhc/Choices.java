...
public class Choices {
    ...
    public Choices(String choicename, int positionX, int positionY, int width, int height, int threshold,
            HeadPositionHCInstance owner) {
        name = choicename;
        fieldWidth = width;
        fieldHeight = height;
        x = positionX;
    ...
    public void draw(Graphics g, IplImage snapImage) {
        ...
        CvScalar color;
        if (selected) {
            color = AbstractCvScalar.RED;
        } else {
            color = AbstractCvScalar.WHITE;
        }
        ...
        cvPutText(snapImage, name, cvPoint(xNamePos, yNamePos), font, AbstractCvScalar.GRAY);
    }
}
