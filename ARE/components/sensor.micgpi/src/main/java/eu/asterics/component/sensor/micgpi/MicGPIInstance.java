...
/**
 * 
 * This pluign delivers data from a sound input device (e.g. standard
 * microphone) It can be used to attach a standard momentary switch to the
 * microphone input and detect button press / release actions or to detect sound
 * events
 * 
 * 
...
public class MicGPIInstance extends AbstractRuntimeComponentInstance implements Runnable {
    ...
    int propMode = 1; // average sample values//average absolute sample
                      // values//min value//max value//max absolute value
    ...
    public Object setRuntimePropertyValue(String propertyName, Object newValue) {
        if ("samplingFrequency".equalsIgnoreCase(propertyName)) {
            ...
            if (propSamplingFrequency < 300) {
                propSamplingFrequency = 300;
            } else if (propSamplingFrequency > 44100) {
                propSamplingFrequency = 44100;
            }
            return oldValue;
        }
        if ("sampleSize".equalsIgnoreCase(propertyName)) {
        ...
        while (running == true) {
            ...
            for (int i = 0; i < readBytes; i++) {
                ...
                case 4: // max absolute value
                    if (Math.abs(value) > inMax) {
                        inMax = Math.abs(value);
                    }
                    break;
                }
            }
            ...
            if (propCalculateFrequency == true) // TBD: make something useful
                                                // with fft data :)
            {
