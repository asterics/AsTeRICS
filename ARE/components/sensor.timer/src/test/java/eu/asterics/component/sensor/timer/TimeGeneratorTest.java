...
public class TimeGeneratorTest {
	...
	/**
	 * Test method for {@link eu.asterics.component.sensor.timer.TimeGenerator#reset()}.
	 */
	@Test
	...
	/**
	 * Test method for {@link eu.asterics.component.sensor.timer.TimeGenerator#start()}.
	 */
	@Test
	...
	/**
	 * This method initializes and start the TimeGenerator instance with the given mode and then checks if the given
	 * expectedEventCount is met.
	 * 
	 * @param tgOther : if null: a new TimeGenerator instance is created, otherwise the given one is reused.
	 * @param mode : {@link TimeGenerator#MODE_ONE_SHOT} or {@link TimeGenerator#MODE_N_TIMES}
	 * @param expectedEventCount : The expected eventCount. If the actual event count != the given expectedEventCount, the test fails
	 * @return
	 */
	private TimeGenerator executeStartOneShotOrNTimes(TimeGenerator tgOther,int mode, int expectedEventCount) {
		...
		if(tg==null) {
			...
			tg=getTimeGenerator(mode,propTimePeriod, 50,0,propRepeatCounter,false,null,new CounterDefaultRuntimeEventTriggererPort());
		}

		tg.start();
		try {
			//Wait double time of timePeriod to ensure the thread had enough time to execute its task
			int waitTime=propTimePeriod*expectedEventCount+propTimePeriod;
			//System.out.println("Waiting "+waitTime);
			Thread.sleep(waitTime);			
		} catch (InterruptedException e) {
			//Should not happen actually, nevertheless it's not totally wrong. Actually the result counts which is checked below.
			e.printStackTrace();
		}
	...
	/**
	 * Test method for {@link eu.asterics.component.sensor.timer.TimeGenerator#stop()}.
	 */
	@Test
	...
	/**
	 * Test method for {@link eu.asterics.component.sensor.timer.TimeGenerator#stopAndSendData()}.
	 */
	@Test
	 ...
	 * @return
	 */
	private TimeGenerator getTimeGenerator(int mode, int propTimePeriod, int propResolution, int propWaitPeriod, int propRepeatCounter, boolean propAutostart, IRuntimeOutputPort opTime, IRuntimeEventTriggererPort etpPeriodFinished) {
...
class CounterDefaultRuntimeEventTriggererPort implements IRuntimeEventTriggererPort {
	...
	public void raiseEvent() {
		synchronized (this) {
			...
			System.out.println(this+".raiseEvent - eventCounter: "+eventCounter+", lastTimeStamp: "+lastTimeStamp);
		}
	}
