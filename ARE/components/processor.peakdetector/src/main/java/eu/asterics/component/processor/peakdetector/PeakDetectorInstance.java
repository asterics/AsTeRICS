...
/**
 * 
 * detects peaks (tops, valleys or both) in an incoming signal outputs events
 * and peak-to-peak timing
 * 
 * 
 * @author <Chris Veigl> [veigl@technikum-wien.at] Date: 02-09-2013 Time:
 */
public class PeakDetectorInstance extends AbstractRuntimeComponentInstance {
    ...
    public Object setRuntimePropertyValue(String propertyName, Object newValue) {
        ...
        if ("comparePeaks".equalsIgnoreCase(propertyName)) {
            ...
            if (propComparePeaks < 0) {
                propComparePeaks = 0;
            }
            if (propComparePeaks > 99) {
                propComparePeaks = 99;
            }
            return oldValue;
        }
    ...
    private final IRuntimeInputPort ipIn = new DefaultRuntimeInputPort() {
        ...
        public void receiveData(byte[] data) {
            ...
            if (actVal > oldVal) {
                actSlope = SLOPE_RISING;
            }
            if (actVal < oldVal) {
                actSlope = SLOPE_FALLING;
            }
    ...
    boolean validBottom(double actBottom) {
        ...
        avg /= propComparePeaks;
        if (actBottom < avg * propValidBottomPercentage / 100.0) {
            return (true);
        }
        return false;
    }
