...
public class CalibrationGenerator implements Runnable {
     ...
     */
    public CalibrationGenerator(final EyeXInstance owner) {
        this.owner = owner;
    ...
    public void run() {
        ...
        for (calibPoint point : calibPoints) {
            ...
            try {
                ...
                // owner.gm.getInstance().calibrationPointStart(point.xLocation,
                // point.yLocation);
                Thread.sleep(800);
    ...
    public void newOffsetPoint(int x, int y, int xOffset, int yOffset) {
        ...
        while (iterator.hasNext()) {
            ...
            if (dist < EyeXInstance.propOffsetPointRemovalRadius) {
                System.out
    ...
    public Point calcOffset(int x, int y) {
        double dist, factor = 0, minDist = EyeXInstance.propOffsetCorrectionRadius;
        ...
        for (calibPoint act : offsetPoints) {
            ...
            if (dist < minDist) {
                minDist = dist;
                factor = 1 - (dist / (float) EyeXInstance.propOffsetCorrectionRadius);
                result = new Point((int) (act.xOffset * factor), (int) (act.yOffset * factor));
            }
        }

        if (minDist != EyeXInstance.propOffsetCorrectionRadius) {
    ...
    public Point getOffsetPoint(int x, int y) {
        double dist, minDist = EyeXInstance.propOffsetCorrectionRadius;
        Point result = new Point(0, 0);
