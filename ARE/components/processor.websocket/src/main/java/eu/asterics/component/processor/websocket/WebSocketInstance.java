...
/**
 * This plugin connects to the WebServiceEngine and sends data to it or receives
 * data from it.
 * 
 * @author Martin Deinhofer [martin.deinhofer@technikum-wien.at] Date: 20140618
 * 
 */
public class WebSocketInstance extends AbstractRuntimeComponentInstance {
    ...
    private final IRuntimeInputPort ipInA = new DefaultRuntimeInputPort() {
        ...
        public void receiveData(byte[] data) {
            // The WebsocketEngine provides the AstericsDataApplication instance
            // which emulates an IRuntimeInputPort
            WebServiceEngine.getInstance().getAstericsApplication().getInputPort("").receiveData(data);
        }
    };

    /**
     * Internal input port that receives the data from the websocket which
     * emulates an IRuntimeOutputPort.
     */
    ...
    /**
     * Registers this instance as an input port receiving data from the
     * WebSocket
     */
    private void register() {
        String componentInstanceId = DeploymentManager.instance
                .getIRuntimeComponentInstanceIDFromIRuntimeComponentInstance(this);
        WebServiceEngine.getInstance().getAstericsApplication().getOutputPort("")
                .addInputPortEndpoint(componentInstanceId, IP_FROM_WEB_SOCKET, ipFromWebSocket, "");
    }
    ...
    private void deRegister() {
        String componentInstanceId = DeploymentManager.instance
                .getIRuntimeComponentInstanceIDFromIRuntimeComponentInstance(this);
        WebServiceEngine.getInstance().getAstericsApplication().getOutputPort("")
                .removeInputPortEndpoint(componentInstanceId, IP_FROM_WEB_SOCKET);
    }
}