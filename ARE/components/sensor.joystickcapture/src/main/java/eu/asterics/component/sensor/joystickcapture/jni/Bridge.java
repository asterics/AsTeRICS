...
public class Bridge
{
    ...
    /**
     * This method is called back from the native code on activation of the bridge. 
     * arguments specify the joystick cabilities
     *
     * @param axis (integer)
     * @param buttons (integer)
     * @param pov (integer)
     * @param min-x axis (integer)
     * @param max-x axis (integer)
     * @param min-y axis (integer)
     * @param max-y axis (integer)
     * @param min-z axis (integer)
     * @param max-z axis (integer)
     * @param min-r axis (integer)
     * @param max-r axis (integer) 
     * @param min-u axis (integer)
     * @param max-u axis (integer)
     * @param min-v axis (integer)
     * @param max-v axis (integer)
     */ 
    private void capabilities_callback(final int axis, final int buttons, final int pov,
    		final int xmin, final int xmax, final int ymin, final int ymax,final int zmin, final int zmax,
    		final int rmin, final int rmax, final int umin, final int umax,final int vmin, final int vmax)
    { 
    	// System.out.println("C++ capabilities callback: Axis="+axis+", Buttons="+buttons+",xmin="+xmin+",xmax="+xmax);
    	owner.axis=axis;
     ...
     *
     * @param x axis (integer)
     * @param y axis (integer)
     * @param z axis (integer)
     * @param r axis (integer)
     * @param u axis (integer)
     * @param v axis (integer)
     * @param buttons  (integer)
     * @param pov value (integer)
     */
    private void newValues_callback( int x, int y, int z, int r, int u, int v,
    		 int buttons,  int pov)
    { 
    	// AstericsErrorHandling.instance.getLogger().fine("Java: Callback [(" + pressState + ", " + keyCode + ")");
    	// System.out.println("C++ values: x="+x+" y="+y+" z="+z+" r="+r+" u="+u+" v="+v);
    	
    	x>>=8;y>>=8;z>>=8;r>>=8;u>>=8;v>>=8;
    	switch (pov) {
			case 0: break; 
			case 4500: pov=1; break; 
			case 9000: pov=2;break; 
			case 13500: pov=3; break;
    	...
    	if (owner.axis>1) owner.opY.sendData(ConversionUtils.intToBytes(y));       
    	if (owner.axis>2) owner.opZ.sendData(ConversionUtils.intToBytes(z));       
    	if (owner.axis>3) owner.opR.sendData(ConversionUtils.intToBytes(r));       
    	if (owner.axis>4) owner.opU.sendData(ConversionUtils.intToBytes(u));       
    	if (owner.axis>5) owner.opV.sendData(ConversionUtils.intToBytes(v));
    	if (owner.hasPov!=0) owner.opPov.sendData(ConversionUtils.intToBytes(pov));
