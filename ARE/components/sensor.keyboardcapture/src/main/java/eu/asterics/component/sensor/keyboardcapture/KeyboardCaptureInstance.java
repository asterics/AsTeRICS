 ...
 *    and detects up to seven command strings which were typed
 *  
...
public class KeyboardCaptureInstance extends AbstractRuntimeComponentInstance
{
	...
	private boolean propBlockEvents = true;
	...
	public void processKeyboardInput (boolean press, int keycode)
	{
		...
		if (press == true)
		{
			...
			//Logger.getAnonymousLogger().info(String.format("keybuffer: %s", act_string));
			...
			if (propMode==MODE_WORDS)
			{
				...
				for (int i=0;i<NUMBER_OF_COMMANDS;i++)
				{
					if (commands[i].equalsIgnoreCase(
							actString.substring(actString.length()-commands[i].length())))
					{
						etpCmdRecognized[i].raiseEvent();
						AstericsErrorHandling.instance.reportInfo(this, String.format("command: %d found, event raised!", i+1));
					}
				}
