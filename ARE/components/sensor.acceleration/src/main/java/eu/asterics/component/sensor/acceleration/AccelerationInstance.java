...
/**
 * Implements the acceleration plugin, which provides data from the acceleration
 * CIM sensor module at 3 output ports (X, Y, Z)
 * 
 * @author Christoph Veigl [veigl@technikum-wien.at] Date: Apr 28, 2011 Time:
 *         11:44:00 AM
 */
public class AccelerationInstance extends AbstractRuntimeComponentInstance implements CIMEventHandler {
    ...
    public synchronized List<String> getRuntimePropertyList(String key) {
        ...
        if (key.compareToIgnoreCase("uniqueID") == 0) {
            ...
            Vector<Long> ids = CIMPortManager.getInstance().getUniqueIdentifiersofCIMs(ACC_CIM_ID);
            if (ids != null) {
                for (Long l : ids) {
            ...
            ids = CIMPortManager.getInstance().getUniqueIdentifiersofWirelessCIMs(WIRELESS_ACC_CIM_ID);
            if (ids != null) {
                for (Long l : ids) {
    ...
    public Object setRuntimePropertyValue(String propertyName, Object newValue) {
        if (KEY_PROPERTY_AUTOSTART.equalsIgnoreCase(propertyName)) {
            ...
            try {
                ...
                AstericsErrorHandling.instance.reportInfo(this,
                        String.format("Setting DescreteSteps to %d", propDescreteSteps));
            ...
            try {
                ...
                AstericsErrorHandling.instance.reportInfo(this,
                        String.format("Setting UpdateFrequency to %d", propUpdateFrequency));
            } catch (NumberFormatException nfe) {
                AstericsErrorHandling.instance.reportInfo(this,
                        "Invalid property value for " + propertyName + ": " + newValue);
            }
            ...
            try {
                ...
                AstericsErrorHandling.instance.reportInfo(this,
                        String.format("Setting AccelerationRange to %d", propAccelerationRange));
            } catch (NumberFormatException nfe) {
                AstericsErrorHandling.instance.reportInfo(this,
                        "Invalid property value for " + propertyName + ": " + newValue);
            }
    ...
    public synchronized void start() {
         ...
         * CIMPortManager.getInstance().getConnection(ACC_CIM_ID); if (port ==
         * null ) { port = CIMPortManager.getInstance()
         * .getWirelessConnection(WIRELESS_ACC_CIM_ID, 0xa000003); if (port !=
         * null) wirelessCIM = true; } else { wirelessCIM = false; }
         */
        if (port == null) {
        ...
        if (port != null) {
            ...
            AstericsErrorHandling.instance.reportError(this,
                    "Could not find Acceleration Module. Please verify that the CIM Module is connected to an USB Port and that the driver is installed.");
        }
        super.start();
    ...
    private void handleAccInputValuePacket(byte[] b) {
        // AstericsErrorHandling.instance.reportInfo(this,
        // "handleAdcInputValuePacket start");
        ...
        total = x * x;
        x -= calX;
        if (propDescreteSteps > 0) {
            x = (x * propDescreteSteps) / 16384;
        }
