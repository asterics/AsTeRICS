...
public class HRVAnalysisInstance extends AbstractRuntimeComponentInstance {
    ...
    private final IRuntimeInputPort ipHRVInput = new DefaultRuntimeInputPort() {
        ...
        public void receiveData(byte[] data) {
            ...
            case LEERLAUFPHASE: // Leerlaufphase = 5sek zum richten vom sensor
                ...
                if (leerlaufphase == 1) {
                    ...
                    leerlaufphase = 0;
                    System.out
                            .println("Leerlaufphase Anfang: " + acttime + "Systemzeit: " + System.currentTimeMillis());
                }
            ...
            case INITPHASE: // analysephase 5sek zum suchen der hoechsten Peaks
                            // und des Grenzwertes

                if (acttime <= init_timer) // Liste wird solange gefuellt wie
                                           // analysephase lauuft
                {
            ...
            case BERG: // Berg
                ...
                if (data_in < maxvalue_border) {
                    ...
                    if (Heartbeat_duration.size() > 4) {
                        if ((heartbeat_duration > Heartbeat_duration_mean * 1.5)
                                || (heartbeat_duration < Heartbeat_duration_mean * 0.5)) {
                    ...
                    if (fl_cap_pos == 0) {
                        ...
                        if (RR_Zeiten.size() > 0) {
                            ...
                            if (peak_time_pos - RR_Zeiten.get(RR_Zeiten.size() - 1) < 150) // werte
                                                                                           // bis
                                                                                           // 200ms
                                                                                           // sind
                                                                                           // moeglich
                        ...
                        System.out.println(
                                "Heartbeat_durationn :" + Heartbeat_duration.get(Heartbeat_duration.size() - 1));
                        fl_cap_pos = 1;
                        System.out.println(
                                "                                                              Pos Spitze gefunden :"
                                        + peak_pos);
            ...
            case TAL: // Tal
                ...
                if (data_in <= minvalue_border) // Tal erkannt, suchen der
                                                // Spitze in in_data_mount List
                {
            ...
            // if (fl_zero_output==0)
            // opPNN50.sendData(ConversionUtils.doubleToBytes(0));
        ...
        public void calc_middle_for_init() {
            if ((minvalue_init * -1) >= maxvalue_init) {
                ...
                System.out.println("Neg ist groesser minvalue :" + minvalue_init + "   maxvalue :" + maxvalue_init
                        + "   middle :" + middle);
            }
            if ((minvalue_init * -1) < maxvalue_init) {
                middle = (maxvalue_init + minvalue_init) / 2;
                System.out.println("Pos ist groesser minvalue :" + minvalue_init + "   maxvalue :" + maxvalue_init
                        + "   middle :" + middle);
            }
        }

        public void check_if_reversed() {
            // Maximalwert im positiven oder negativen Bereich, bzw. sensor
            // verkehrt?
            if ((minvalue_init * -1) > maxvalue_init) {
                fl_twist_data_in = 1;
                ;
        ...
        public void calc_peakmean_pos() {
            ...
            if (data_peaks_pos.size() > 0) {
                if (data_peaks_pos.size() < 5)// wenn peakcounter kleiner als 5
                {
                    ...
                    maxvalue_border = (peak_mean_pos / 100) * 70;// neue Grenze
                                                                 // wird gesetzt
                    System.out.println("maxvalue_border nach peakmean : " + maxvalue_border);

                } else // wenn peakcounter > als 4
                {
                    System.out.println("wenn peakcounter groesser als 4    maxsize : " + data_peaks_pos.size()
                            + "maxsize : " + (data_peaks_pos.size() - 5));
                    ...
                    // peak_mean_pos_sum +=
                    // data_peaks_pos.get(data_peaks_pos.size()-1);
        ...
        public void calc_RRintervals() { // Berechnenen der Intervalle aus den
                                         // Zeiten
        ...
        public void calc_heartbeat_duration() { // Berechnung
                                                // Heartbeat_duration_mean
        ...
        public void calc_peakmean_neg() {
            ...
            if (data_peaks_neg.size() > 0) {
                if (data_peaks_neg.size() < 5)// wenn peakcounter kleiner als 5
                {
                    ...
                    minvalue_border = (peak_mean_neg / 100) * 70;// neue Grenze
                                                                 // wird gesetzt
    ...
    public void start() {
         ...
         * CIMPortManager.getInstance().sendPacket(portController,turn_on,
         * (short) 0,(short) 0, false); /*public int
         * sendPacket(CIMPortController ctrl, byte [] data, short
         * featureAddress, short requestCode, boolean crc)
         */
        /*
         * in = portController.getInputStream(); readerThread = new Thread(new
         * Runnable() {
         * 
         * @Override public void run() { running = true; System.out.println(
         * "truesense in read thread");
         * 
         * while (running) {
         * 
         * try { while (in.available() > 0) {
         * 
         * handlePacketReceived((byte) in.read()); //System.out.println(
         * "in.read() "+in.read()); } System.out.println(
         * "truesense request data"); //Thread.sleep(100);
         * CIMPortManager.getInstance().sendPacket(portController,request_data,
         * (short) 0,(short) 0, false); Thread.sleep(30); } catch
