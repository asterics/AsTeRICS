...
public class openBCIInstance extends AbstractRuntimeComponentInstance {
     ...
     * 
     * void parsePacket() { try { int x = 2; for (int k = 0 ; k < 6; k++) {
     * value[k]=(((int) array[x+1]) & 0xff) | ((((int) array[x]) & 0xff) << 8);
     * 
     * // value[k] =(int)( (array[x] << 8) + array[x+1]); x++; x++; }
     * 
     * // System.out.println("Read values: Channel1 " + value[0] +",Channel2: "
     * + value[1]+",Channel3: " + value[2]+",Channel4: " + value[3]+
     * ",Channel5: " + value[4]+",Channel6: " + value[5]);
     * opChannel1.sendData(value[0]-512); opChannel2.sendData(value[1]-512);
     * opChannel3.sendData(value[2]-512); opChannel4.sendData(value[3]-512);
     * opChannel5.sendData(value[4]-512); opChannel6.sendData(value[5]-512);
     * opChannel7.sendData(value[5]-512); opChannel8.sendData(value[5]-512);
     * 
     * } catch (Exception e) { // happens when Deque does not contain full
     * packet yet } }
     */
     ...
     * 
     *            https://github.com/OpenBCI/OpenBCI/wiki/Data-Format-for-
     *            OpenBCI-V3
     * 
     *            3-byte signed integers are stored in 'big endian' format, MSB
     *            first. The 3-byte values are converted to 4-byte signed longs
     *            during the unpacking.
     * 
     *            Accelerometer data are 2-byte signed ints. Also converted to
     *            signed longs.
     * 
     *            Start Indicator: 0xA0 Frame number : 1 byte (advances on each
     *            packet) Channel N data : 3 bytes per channel (repeats for ch
     *            1-8, or ch 1-16) ... A_channel X : 2 bytes (note these may
     *            also be instead, optional user data, A_channel Y : 2 bytes
     *            replacing accelerometer values.) A_channel Z : 2 bytes End
     *            Indcator: 0xC0
     * 
     * 
     */
    ...
    public void handlePacketReceived(byte actbyte) {
        ...
        // To better sync up when lost, look for two byte sequence. It has
        // happened
        // previously, that when data contains lots of A0's, sync could not
        // reestablish.
        //
        case 0:
            if ((0x000000ff & actbyte) == 0xC0) {
                state++;
            }
            break;

        case 1:
            if ((0x000000ff & actbyte) == 0xA0) {
                state++;
            } else {
                state = 0;
            }
        ...
        case 2:
            if ((0x000000ff & actbyte) != framenumber) {
                syncloss++;
        ...
        case 3: // get channel values
            tempval |= ((0x000000ff & actbyte) << (16 - (bytecounter * 8))); // big
                                                                             // endian
            bytecounter++;
            if (bytecounter == 3) {
                ...
                case 0:
                    opChannel1.sendData(tempval);
                    break;
                case 1:
                    opChannel2.sendData(tempval);
                    break;
                case 2:
                    opChannel3.sendData(tempval);
                    break;
                case 3:
                    opChannel4.sendData(tempval);
                    break;
                case 4:
                    opChannel5.sendData(tempval);
                    break;
                case 5:
                    opChannel6.sendData(tempval);
                    break;
                case 6:
                    opChannel7.sendData(tempval);
                    break;
                case 7:
                    opChannel8.sendData(tempval);
                    break;
                }
        ...
        case 4: // get accelerometer XYZ
            tempval |= ((0xff & actbyte) << (8 - (bytecounter * 8))); // big
                                                                      // endian
            bytecounter++;
            if (bytecounter == 2) {
                ...
                switch (channelcounter) {
                case 8:
                    opAccX.sendData(tempval);
                    break;
                case 9:
                    opAccY.sendData(tempval);
                    break;
                case 10:
                    opAccZ.sendData(tempval);
                    break;
                }
                channelcounter++;
        ...
        case 5:
            if ((0x000000ff & actbyte) == 0xC0) // if correct end delimiter
                                                // found:
            {
                state = 1;
            } else {
    ...
    public void resume() {
        ...
        if (portController == null) {
            AstericsErrorHandling.instance.reportError(this,
                    "Could not construct openBCI raw port controller, please make sure that the COM port is valid.");
        }
        readerThread.start();
        super.resume();
    }
