...
/**
 * 
 * The SpeechProcessor plugin builds a brigde to the SpeechProcessor.exe
 * program (path can be given a s plugin property, default = ./tools/SpeechProcessor.exe)
 * which provides an interface to the Microsoft Speech Platfrom (version 11)
 * for speech recognition and generation.
 * 
 * The plugin builds a recognition grammar for up to n speech commands given as properties
 * and allows to trigger events upon recogintion. Additionally the speech synthesizer can
 * be used via an input port for strings.
 * 
 * A language (culture) can be selected also via a property, the corresponging laguage packs
 * for TTS and SR for 26 languages can be downloaded via the following links:
 * 
...
public class SpeechProcessorInstance extends AbstractRuntimeComponentInstance
{
	...
	private boolean speechProcessorOK = false;
    ...
    */
    ...
    public Object setRuntimePropertyValue(String propertyName, Object newValue)
    {
		...
		if ("mode".equalsIgnoreCase(propertyName))
		{
			...
			if (propMode==MODE_ALWAYSACTIVE) recoActive = true;
			else recoActive = false;
	...
	private final IRuntimeInputPort ipSpeak  = new DefaultRuntimeInputPort()
	{
		public void receiveData(byte[] data)
		{
			...
			boolean ret = RemoteConnectionManager.instance.writeData(
	...
	final IRuntimeEventListenerPort elpDeActivate = new IRuntimeEventListenerPort() {
      ...
      */
      ...
      public void start()
      {
    	  ...
    	  {
    		  AstericsErrorHandling.instance.reportInfo(this, "Starting " 
    				  + speechProcessorExePath);
	    		  ...
	    		  AstericsThreadPool.instance.execute(new Runnable() {
	    			public void run()
	    			{
	    				try
	    				{
	    					...
	    					{
	    				   		  AstericsErrorHandling.instance.reportError(instance, "Could not connect to SpeechRecognizer ("
	   				   			+ speechProcessorExePath+").");
	    					}
	    					if (speechProcessorOK==false)
	    					{
	    				   		  AstericsErrorHandling.instance.reportError(instance, "Could not activate Speech Synthesizer and Recognizer.");
	    					}  					
	    				}
	    				catch (InterruptedException e) {}
 ...
 */
    		  ...
    		  {
    			  AstericsErrorHandling.instance.reportError(this, "Could not start " 
        				  + speechProcessorExePath);
    			  ioe.printStackTrace();
    		  }
      ...
      public void stop()
      {
		...
		"stop()");
    	  ...
    	  if (connectionEstablished==true)
    	  {
				...
				RemoteConnectionManager.instance.writeData(
						Integer.toString(tcpPort), message.getBytes());
	...
	{
		AstericsErrorHandling.instance.reportInfo(instance,
				String.format("Closing Connection to SpeechProcessor"));
   ...
   public void tellWords()
   {
	 ...
	 * communication with the PSeechProcessor.
	...
	class SpeechProcessorConnectionListener implements IRemoteConnectionListener
	{
		...
		/**
		 * Called once a connection is set up on a TCP port. Initiates the Recognizer Culture,
		 * Grammar and the speech commands.
		 */
		public void connectionEstablished()
		{
			...
			if (propActivationCommand.length()<2) propActivationCommand="activate";
			if (propDeActivationCommand.length()<2) propDeActivationCommand="deactivate";
			if (propHelpCommand.length()<2) propHelpCommand="help";
			...
			switch (propLanguage)
			{
				...
				case 1:message+="de-DE"; break;
				case 2:message+="es-ES"; break;
				case 3:message+="pl-PL"; break;
		...
		public void dataReceived(byte [] data)
		{
			...
			AstericsErrorHandling.instance.reportInfo(instance,
					String.format("SpeechProcessor: data: %s",input));
			

			if (input.equals("@SpeechProcessor OK@")) 
				speechProcessorOK = true;
			...
			if (recoActive == true) 
			{
				...
				else if ((propMode == MODE_VOICETRIGGERED) && 
						( compareCommand(propDeActivationCommand,input)))
				{
					recoActive=false;						
					etpDeActivated.raiseEvent();
				}
		...
		/**
		 * Compares recognized command sent by the SpeechProcessor with the property command.
		 */
