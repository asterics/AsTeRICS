...
/**
 * Implements the Oscilloscope plugin, which can display one or two signal
 * traces in a GUI
 * 
 * @author Armin Schmoldas [armin.schmoldas@technikum-wien.at] Date: Apr 21,
 *         2015 Time: 1:00:05 PM
 */
public class MotionAnalysisInstance extends AbstractRuntimeComponentInstance {
    ...
    /**
     * Checks how many channels are connected. After all channels have sent data
     * the draw channel is updated.
     */
    ...
    /**
     * Calculates the relative difference between the load channel and the draw
     * channel.
     */
    public void compare() {
        if (gui.loadchnValues.length != 0) {
            ...
            if (propMax - propMin != 0) {
                for (int i = 0; i < gui.allloadValues.size(); i++) {
                    percent.add(Math.abs(gui.alldrawValues.get(i) - gui.allloadValues.get(i))
                            / Math.abs(propMax - propMin));
                }
            } else {
    ...
    /**
     * Sends the actual position of the exercise in percent to the percent
     * output port.
     */
    public void percent() {
