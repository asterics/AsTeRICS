...
//	public double chnMax = Double.MIN_VALUE;
	...
	class PanelWithOsciGraph extends JPanel {
		...
		/**
		 * Draws all the graphics of the graph like the line in the middle
		 * and sets the colors of it and the background color
		 */
		...
		public synchronized void paintComponent(Graphics g) {
			...
			if (owner.propMax!=owner.propMin) drawFactorY=h/(owner.propMax-owner.propMin);
			...
			catch (ArrayIndexOutOfBoundsException e) { System.out.println("exception in paint paintComponent, MotioinAnalysis"); }
			try {
	...
	{
		if (owner.propMin>actValue) owner.propMin=actValue;
		if (owner.propMax<actValue) owner.propMax=actValue;
	...
	synchronized void updatedrawChn(double actValue)
	{
		...
		else
		{
			...
			{
				if (owner.propMin>actValue) actValue=owner.propMin;
				if (owner.propMax<actValue) actValue=owner.propMax;
			}
		}
		...
		// if a exercise is done and the list of load values is the same length of the list of draw values
		// the exercises is stopped and the compare function is called
		
		if (save == false)
		{
			if (roundloadValues.get(loadcount-1).length == chndrawPos && roundloadValues.get(loadcount-1).length != w-1)
			{
				owner.pause();
		...
		// if the draw channel comes to the end of the graph the position is set to 0 
	...
	synchronized void startwrite()
	{
		...
		try {
			...
			dir.mkdir();
			out = new BufferedWriter(new FileWriter(owner.propFilepath + owner.propFilename + "_" + sdf.format(cal.getTime()) + ".csv"));
		} catch (IOException e) {
	...
	/**
	 * changes a double backslash to 4 backslashes
	 * this is needed in java to load a file 
	 */
