...
/**
 * 
 * EventStateMachine plugin compares incoming events with a given sequence fires
 * event when sequence is complete optionally resets to inital state when
 * non-matching event comes in
 * 
 * @author Chris Veigl [veigl@technikum-wien.at] Date: 2013-05-26 Time:
 */
public class EventStateMachineInstance extends AbstractRuntimeComponentInstance {
    ...
    void initSequence() {
        ...
        for (int i = 0; i < states.length; i++) {
            ...
            // System.out.println("State "+(i+1)+": Event "+states[i]+",
            // minTime="+mins[i]+", maxTime="+maxs[i]);
        }
    }
    ...
    final synchronized void checkEvent(String actEvent) {
        ...
        if (!(states[currentState].equals(actEvent))) {
            ...
            if (((propSkipFirstEvent == false) && (currentState > 0)) || (currentState > 0)) {
                ...
                if ((mins[currentState - 1] != -1) && (mins[currentState - 1] > currentTime)) {
                    stateError = true;
                }
                if ((maxs[currentState - 1] != -1) && (maxs[currentState - 1] < currentTime)) {
                    stateError = true;
                }
            }
        }
