...
/**
 * OskaCommunication performs all the actions that are directly connected to the
 * OSKA third party application. This includes opening and closing the
...
class OskaCommunication {
     ...
     *         and handled through the OSKA connection listener
     */
    ...
    class OskaConnectionListener implements IRemoteConnectionListener {
         ...
         * 
         * @param owner
         *            the OSKA component instance owning the listener
         */
        ...
        public void connectionEstablished() {
            AstericsErrorHandling.instance.reportInfo(OskaInstance.instance,
                    String.format("Connection established via OSKA " + "connection listener"));
            connectionEstablished = true;
        ...
        public void dataReceived(byte[] data) {
            AstericsErrorHandling.instance.getLogger()
                    .fine(String.format("OSKA connection listener: data: %s", new String(data)));
        ...
        public void connectionLost() {
            connectionEstablished = false;
            AstericsErrorHandling.instance.reportInfo(OskaInstance.instance,
                    String.format("Connection has been lost claims OSKA connection " + "listener"));
        }
     ...
     * 
     * @param message
     *            the command to be sent to OSKA
    ...
    boolean sendToOska(String message) {
        ...
        if (connectionEstablished) {
            if (useDoubleQuotes) {
                ...
                cmd.append("\"");
                AstericsErrorHandling.instance.reportDebugInfo(OskaInstance.instance,
                        String.format("Sending to port %d: %s", tcpPort, cmd.toString()));
                ret = RemoteConnectionManager.instance.writeData(Integer.toString(tcpPort), cmd.toString().getBytes());
            } else {
                AstericsErrorHandling.instance.reportDebugInfo(OskaInstance.instance,
                        String.format("Sending to port %d: %s", tcpPort, message));
            }
            ret = RemoteConnectionManager.instance.writeData(Integer.toString(tcpPort), message.getBytes());
            try {
    ...
    boolean loadKeyboard(String kbdPath) {
        if (!kbdPath.trim().equals("")) {
            AstericsErrorHandling.instance.reportInfo(OskaInstance.instance, String.format("Loading kbd: %s", kbdPath));
