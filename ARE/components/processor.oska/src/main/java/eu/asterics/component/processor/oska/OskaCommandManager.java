...
class OskaCommandManager {
    ...
    Hashtable<String, IOskaCommandHandler> commands = new Hashtable<String, IOskaCommandHandler>();

    Hashtable<String, IOskaActionStringHandler> actionStrings = new Hashtable<String, IOskaActionStringHandler>();
     ...
     *            the command name
     * @param handler
     *            the handler implementation for said command
     */
    ...
    /**
     * Adds a handler for a certain action string to the list of known action
     * strings
     * 
     * @param cmd
     *            the command name
     ...
     * 
     * @param input
     *            the string received from the OSKA
     ...
     * 
     * @param input
     *            the input from OSKA or the action string if it could be
     *            extracted
     */
        ...
        if (input[0].trim().startsWith("COMMAND:")) {
            ...
            if (input.length > 3) {
                ...
                for (String actionString : actions) {
                    ...
                    IOskaActionStringHandler handler = actionStrings
                            .get((colonIdx != -1) ? actionString.substring(0, colonIdx) : actionString);
                    ((handler != null) ? handler : defaultHandler).handleActionString(actionString);
