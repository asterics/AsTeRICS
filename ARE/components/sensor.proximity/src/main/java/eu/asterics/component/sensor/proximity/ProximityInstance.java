 ...
 * 
 * 
 * @author Franziska Horak [franziska.horak@gmail.com] Date: 07.March 2013 Time:
 *         14:15
 */
public class ProximityInstance extends AbstractRuntimeComponentInstance implements CIMEventHandler {
    ...
    private static final short TEENSY_CIM_FEATURE_SET_THRESHOLD = 0x02; // Schwellwert
                                                                        // setzen
    private static final short TEENSY_CIM_FEATURE_MODE_SELECTION = 0x06; // Auswahl
                                                                         // des
                                                                         // Sendemodus
     ...
     */
    @Override
    public Object setRuntimePropertyValue(String propertyName, Object newValue) {// Werte
                                                                                 // kommen
                                                                                 // von
                                                                                 // ACS
                                                                                 // in
    ...
    public void start() {
        ...
        if (port != null) {
            ...
            // "ProximityInInstance started");
        }

        else {
            AstericsErrorHandling.instance.reportError(this,
                    "Could not find Teensy controller, make sure the Proximity module is connected to an USB port and correctly installed.");
        }
    }
    ...
    public void resume() {
        port.addEventListener(this);
        super.resume();
        // AstericsErrorHandling.instance.reportInfo(this, "ProximityInInstance
        // resumed");
    }
    ...
    public void handlePacketReceived(CIMEvent e) {
        ...
        if (((packet.getSerialNumber() & 0x80) != 0) && (packet.getFeatureAddress() == 0x03)) // CIM_FEATURE_SEND_EVENT
        {
            ...
            if (b[0] == 1) {
                // AstericsErrorHandling.instance.reportInfo(this, "Raising high
                // trigger on " + i);
                etpLowToHigh.raiseEvent();
            } else {
                // AstericsErrorHandling.instance.reportInfo(this, "Raising low
                // trigger on " + i);
        ...
        {
            byte[] b = packet.getData();
            // AstericsErrorHandling.instance.reportInfo(this, "Got periodic
            // update !");
