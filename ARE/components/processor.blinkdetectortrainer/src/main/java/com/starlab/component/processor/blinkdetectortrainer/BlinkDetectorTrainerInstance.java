...
/**
 * 
 * This module implements the training of the blink detector plug-in by
 * measuring the maximum and minimum thresholds, the blink length and the
 * separation between double blinks for each subject.
 * 
 * 
 * 
 * @author Laura Dubreuil Vall [laura.dubreuil@starlab.es] Date: 03/09/2012
 *         Time: 13:41
 */
public class BlinkDetectorTrainerInstance extends AbstractRuntimeComponentInstance {
    ...
    // Index for total blinks (each blink of double blinks are also considered
    // simple blinks)
    int iMinTotal = 0;
    int iMaxTotal = 0;
    ...
    public native int nativeComputeSampleBlinkDetectorTrainer(long handle, double sample, int firstSample,
            int lookForSimple, int lookForDouble, int index, int iMinTotal, int iMaxTotal);

    public native double nativeComputeMax(long handle, int size); // Compute
                                                                  // minimum max

    public native double nativeComputeMin(long handle, int size); // Compute
                                                                  // maximum min

    public native int nativeComputeLength(long handle, int size); // Simple
                                                                  // blink
                                                                  // length

    public native int nativeComputeSeparation(long handle, int size); // Double
                                                                      // blink
                                                                      // separation

    public native int nativeSetParametersDoubleBlink(long handle, double minThreshold, double maxThreshold,
    ...
    private final IRuntimeInputPort ipInput = new DefaultRuntimeInputPort() {
        ...
        public void receiveData(byte[] data) {
            ...
            synchronized (lock) {
                firstSample = nativeComputeSampleBlinkDetectorTrainer(blinkDetectorHandle, in, firstSample,
                        lookForSimple, lookForDouble, index, iMinTotal, iMaxTotal);
                ...
                public void run() {
                    ...
                    case 12:
                        if (counter >= 1) {
                            ...
                            firstSample = 1;
                            /*
                             * double maximumSimple =
                             * 0.3*nativeComputeMax(blinkDetectorHandle,5);
                             * double minimumSimple =
                             * 0.3*nativeComputeMin(blinkDetectorHandle,5); int
                             * interval1 =
                             * 5*nativeComputeLength(blinkDetectorHandle,5); int
                             * interval2 = (int)(interval1*3);
                             * System.out.println(
                             * "Parameters for double blink: ");
                    ...
                    case 22:
                        ...
                        if ((maximum != 0) && (minimum != 0) && (length != 0) && (separation != 0)) {
                            results = "Maximum threshold: " + Integer.toString(maximum) + "        Minimum threshold: "
                                    + Integer.toString(minimum) + "        Blink length: " + Integer.toString(length)
                                    + " ms" + "        Double blink separation: " + Integer.toString(separation)
                                    + " ms";
                        } else {
