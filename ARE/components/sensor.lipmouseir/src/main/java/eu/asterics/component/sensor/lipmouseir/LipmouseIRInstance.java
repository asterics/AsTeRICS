 ...
 * 
 * 
 * @author Alberto Ibanez [alberto_21_9@hotmail.com], extended by Christoph
 *         Ulbinger [christoph.ulbinger@technikum-wien.at] Date: 08.04.2015
 *         Time: 09:14
 */
public class LipmouseIRInstance extends AbstractRuntimeComponentInstance implements CIMEventHandler {
    ...
    private static final short LIPMOUSE_CIM_FEATURE_SET_IR_MODE = 0x0008; // Set
                                                                          // Lipmouse
                                                                          // into
                                                                          // sending/receiving
                                                                          // mode
    private static final short LIPMOUSE_CIM_FEATURE_GET_IR_STATE = 0x0009; // Get
                                                                           // state
                                                                           // of
                                                                           // the
                                                                           // Lipmouse
    private static final short LIPMOUSE_CIM_FEATURE_SET_IR_CODE = 0x000A; // Send
                                                                          // IR
                                                                          // code
                                                                          // to
                                                                          // Lipmouse
    private static final short LIPMOUSE_CIM_FEATURE_GET_IR_CODE = 0x000B; // Receive
                                                                          // IR
                                                                          // code
                                                                          // from
                                                                          // Lipmouse
    ...
    final IRuntimeEventListenerPort elpSendIRCode = new IRuntimeEventListenerPort() {
        ...
        public void receiveEvent(final String data) {
            ...
            if ((DeviceType != "") && (DeviceName != "") && (DeviceFunction != "")) {
                ...
                if (port != null) {
                    CIMPortManager.getInstance().sendPacket(port, IRCode, LIPMOUSE_CIM_FEATURE_SET_IR_CODE,
    ...
    final IRuntimeEventListenerPort elpRecordIRCode = new IRuntimeEventListenerPort() {
        ...
        public void receiveEvent(final String data) {
            if ((DeviceType != "") && (DeviceName != "") && (DeviceFunction != "")) {
                ...
                sendLipmouseWriteFeature(LIPMOUSE_CIM_FEATURE_SET_IR_MODE, 2); // 0
                                                                               // :
                                                                               // idle
                                                                               // 1
                                                                               // :
    ...
    private void ShowRecordMode() {
        etpStartRecord.raiseEvent(); // raise event to show that Lipmouse is
                                     // recording
    }

    private void HideRecordMode() {
        etpStopRecord.raiseEvent(); // raise event to show that Lipmouse has
                                    // finished recording
    }
    ...
    private void handleLipmouseUniqueNumber(CIMProtocolPacket packet) // The
                                                                      // Lipmouse
                                                                      // will
                                                                      // never
                                                                      // calls
    ...
    private void handleIRState(byte[] packet) {
        ...
        case 4: // Lipmouse received IR Code from ARE
            ...
            sendLipmouseWriteFeature(LIPMOUSE_CIM_FEATURE_SET_IR_MODE, 1); // 0
                                                                           // :
                                                                           // idle
                                                                           // 1
                                                                           // :
    ...
    private void handleReceivedIRCode(byte[] packet) {
        ...
        String IR_Code = "";
        for (int i = 0; (i < IR_Data.length) && (i < DATABUF_SIZE); i++) // create
                                                                         // a
                                                                         // CSV
                                                                         // format
        {
    ...
    private byte[] ParseIRCode(String IR_Code_Line) // Parse IR code reveived
                                                    // from database
    {
        String tmpCode[] = IR_Code_Line.split(",");
        byte IRCode[] = new byte[DATABUF_SIZE];

        for (int counter = 0; (counter < (tmpCode.length - 3)) && (counter < DATABUF_SIZE); counter++) // -3
                                                                                                       // due
                                                                                                       // to
                                                                                                       // DeviceType,
                                                                                                       // Devicename,
    ...
    public String ReadFromCSV() // Read IR code from database
    {
        ...
        try {
            ...
            if (currentLine == "") {
                System.out.println(
                        "#Error: Could not find IR Code for " + DeviceType + " " + DeviceName + " " + DeviceFunction);
                return null;
            }
            in.close();
        } catch (IOException e) {
    ...
    public void WriteToCSV(String IRCode) // Write IR code to database
    {
        checkPath();
        File csvfile = new File(propIRCodeFilePath);
        File tmp_csvfile = new File("tmp_IRCodes.csv"); // create a temporary
                                                        // file to reduce RAM
                                                        // usage
        BufferedReader in;
        BufferedWriter out;
        ...
        try {
            ...
            out.write(DeviceType + "," + DeviceName + "," + DeviceFunction + "," + IRCode
            ...
            if (tmp_csvfile.renameTo(csvfile)) {
                System.out.println("Write " + DeviceType + " " + DeviceName + " " + DeviceFunction + " to file "
                        + propIRCodeFilePath);
            } else {
    ...
    public void start() {
        ...
        if (port != null) {
            ...
            AstericsErrorHandling.instance.reportError(this, "Could not find LipMouse Module with IR Module (ID "
                    + propUniqueID
    ...
    synchronized private final void sendLipmouseReadFeature(short feature, int value) {
        ...
        if (port != null) {
            // System.out.println("sending lipmouse-packet !");
            CIMPortManager.getInstance().sendPacket(port, b, feature, CIMProtocolPacket.COMMAND_REQUEST_READ_FEATURE,
                    false);
        }
    }
}