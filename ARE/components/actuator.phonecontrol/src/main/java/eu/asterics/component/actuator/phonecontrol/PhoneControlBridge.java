...
public class PhoneControlBridge implements Runnable {
     ...
     * 
     * @param opReceivedSMS
     *            SMS content output port
     * @param phoneIDOut
     *            phone ID output port
     * @param opRrrorNumber
     *            error number output port
     * @param etpIdleState
     *            phone idle output event port
     * @param etpRingState
     *            phone ring output event port
     * @param etpConnectedState
     *            phone connected output event port
     * @param etpNewSMS
    ...
    public PhoneControlBridge(final PhoneControlInstance.OutputPort opReceivedSMS,
            final PhoneControlInstance.OutputPort phoneIDOut, final PhoneControlInstance.OutputErrorPort opErrorNumber,
            final IRuntimeEventTriggererPort etpIdleState, final IRuntimeEventTriggererPort etpRingState,
            final IRuntimeEventTriggererPort etpConnectedState, final IRuntimeEventTriggererPort etpNewSMS,
            final IRuntimeEventTriggererPort etpError) {
    ...
    public void start() {
        ...
        if (phoneName.length() > 0) {
            AstericsErrorHandling.instance.getLogger()
                    .fine("activate: " + phoneName + "port: " + Integer.toString(phonePort));
            ...
            if (result < 0) {
                if (result == Phone_Not_Found) {
                    ...
                    AstericsErrorHandling.instance.getLogger()
                            .warning("Phone initializing error: " + Integer.toString(result));
                }
            }
        } else {
    ...
    public void makePhoneCall() {
        ...
        /*
         * result = makePhoneCall(phoneID); if(result<0) {
         * opRrrorNumber.sendData(result); etpError.raiseEvent(); }
         */
    }
    ...
    public void sendSMS() {
        ...
        /*
         * result = sendSMS(phoneID,SMSContent); if(result<0) {
         * opRrrorNumber.sendData(result); etpError.raiseEvent(); }
         */
    }
    ...
    public void reconnect() {
        ...
        if (result < 0) {
            if (result == Library_No_Initialized) {
                ...
                if (result < 0) {
                    AstericsErrorHandling.instance.getLogger()
                            .warning("Phone reconnect error: " + Integer.toString(result));
                    opErrorNumber.sendData(result);
                    etpError.raiseEvent();
                }
            } else {
     ...
     *
     * @param phoneBTName
     *            the Bluetooth name of the phone to be accessed
     * @param portNumber
     *            the phone port Number
     ...
     *
     * @param phoneState
     *            is the current phone state
     * @param phoneID
     *            is the caller phone ID
     */
     ...
     *
     * @param result
     *            is the connection establish result
     */
