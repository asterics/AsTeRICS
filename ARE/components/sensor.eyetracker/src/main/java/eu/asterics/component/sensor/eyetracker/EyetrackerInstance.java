...
import eu.asterics.component.sensor.eyetracker.CalibrationGenerator;
...
/**
 *   Implements the eyetracker plugin, which uses OpenCV 
 *   to detect a user's eye movement recorded by a head mounted 
 *   webcamera with infrared light support (dark pupil method) 
 *  
   ...
   public Object setRuntimePropertyValue(String propertyName, Object newValue)
    {
       if("trackingMode".equalsIgnoreCase(propertyName))
       {
           ...
           if ((lastMode == MODE_EYETRACKING_HEADPOSE) && (propTrackingMode != MODE_EYETRACKING_HEADPOSE))
        	   positObj.pausePosit();
           else if ((lastMode == MODE_BLOBTRACKING || lastMode == MODE_EYETRACKING) && (propTrackingMode == MODE_EYETRACKING_HEADPOSE))
     ...
     */
    private final IRuntimeInputPort ipPt1x
            = new DefaultRuntimeInputPort(){    };
    private final IRuntimeInputPort ipPt1y
            = new DefaultRuntimeInputPort(){    };
    private final IRuntimeInputPort ipPt2x
            = new DefaultRuntimeInputPort(){    };
    private final IRuntimeInputPort ipPt2y
            = new DefaultRuntimeInputPort(){    };
    private final IRuntimeInputPort ipPt3x
            = new DefaultRuntimeInputPort(){    };
    private final IRuntimeInputPort ipPt3y
            = new DefaultRuntimeInputPort(){    };
    private final IRuntimeInputPort ipPt4x
            = new DefaultRuntimeInputPort(){    };
    private final IRuntimeInputPort ipPt4y
            = new DefaultRuntimeInputPort(){    };
    
    
	@Override
	public void syncedValuesReceived(HashMap<String, byte[]> dataRow) {
//		System.out.println("syncdValuesReceived" + dataRow.keySet().toString());
		...
		else  // calibrated eyetracking (with or without head pose correction)
		{
	    	switch (state) 
	    	{
		    	case STATE_IDLE:
		    		...
		    		if (bypassBlinks==0)
		    		{
			    		if ((propMinBlinkDuration>0)&&(idleTime>propMinBlinkDuration) && (idleTime<propMaxBlinkDuration))
		    	...
		    	case STATE_RUNNING:
		    		...
		    		{
						//use the head pose for pose compensation based on pose during each calib step:
