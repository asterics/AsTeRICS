...
public class POSIT {
    ...
    volatile private float tvecx, tvecy, tvecz; // translation and rotation
                                                // vector for data exchange
    volatile private float rvecx, rvecy, rvecz; // units for translation: mm;
                                                // for rotation: rad

    final Lock tvlock = new ReentrantLock(); // locks to ensure safe write and
                                             // read of the vectors
    ...
    private float calibAvgTvecx = 0, calibAvgTvecy = 0, calibAvgTvecz = 0; // average
                                                                           // values
                                                                           // of
                                                                           // the
                                                                           // vectors
    ...
    /**
     * starts the evaluation of the systems accuracy sets switch to send the eye
     * coordinates to the native code
     */
    protected synchronized void startEvaluation() {
        ...
        if (errormsg != 1) {
            AstericsErrorHandling.instance.reportInfo(this.owner,
                    String.format("could not start evaluation: %x", errormsg));
        }
    }
    ...
    public synchronized void stopSendEyeCoordinates() {
        ...
        // String.format("JVM: stopped sending eye coordinates to native
        // code"));
    ...
    protected void togglePoseInfoWindow() {
        ...
        if (errormsg != 1) {
            AstericsErrorHandling.instance.reportInfo(this.owner,
                    String.format("could not toggle Info Window: %x", errormsg));
        }
    }
     ...
     * 
     * @param resH
     *            horizontal display resolution (i.e. 1920)
     * @param resV
     *            vertical display resolution (i.e. 1080)
     * @param size
     *            diagonal display size in inch / "Zoll" (i.e. 24")
     */
    ...
    /**
     * calculates the average pose with the previously stored rotation and
     * translation vectors (stored in calibTRVecMatrix)
     * 
     * @param column
     *            amount of columns-1
     * @param row
    ...
    /**
     * calculates the x and y deviation due to head movements translation and
     * rotation around x and y axis are considered (rotation around the z axis
     * is not considered) calibratedX and calibratedY of eyetrackerInstance are
     * corrected with the deviation
     */
    protected void doSimplePOSITCorrection() {
        ...
        // deltaX = (Math.tan(tmp.rvecy)*(tmp.tvecz-70)) -
        // (Math.tan(calibAvgRvecy)*(calibAvgTvecz-70));
        // deltaY = (Math.tan(-tmp.rvecx)*(tmp.tvecz-70)) -
        // (Math.tan(-calibAvgRvecx)*(calibAvgTvecz-70));

        // ******* addition to method 1: translation can be added to the
        // calculation for smoothing the result
        // deltaX = (deltaX + 2*(tmp.tvecx-calibAvgTvecx)) / 3;
        // deltaY = (deltaY + 2*(tmp.tvecy-calibAvgTvecy)) / 3;

        // ******* Method 2: calculate the correction factor with the
        // translation
    ...
    private double calcAvgDeltaX(double x) {
        ...
        if (cntDeltaX_array == (avgArraySize - 1)) {
            cntDeltaX_array = 0;
        } else {
            cntDeltaX_array++;
        }

        for (int i = 0; i < avgArraySize; i++) // sum values, search for highest
                                               // and lowest value
        {
        ...
        avgX = ((avgX - low) - high) / (avgArraySize - 2); // calculate average
                                                           // without highest
                                                           // and lowest value
        return avgX;
    }
    ...
    public void sendEvalValues() {
        ...
        // AstericsErrorHandling.instance.reportDebugInfo(this.owner,
        // String.format("sending eval values"));
    }
