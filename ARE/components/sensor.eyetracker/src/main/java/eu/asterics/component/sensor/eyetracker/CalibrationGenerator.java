...
public class CalibrationGenerator implements Runnable {
    ...
    public void updateCalibParams(int propXMin, int propXMax, int propYMin, int propYMax, int propCalibColumns,
            int propCalibRows) {
        xMin = propXMin;
        xMax = propXMax;
        yMin = propYMin;
        ...
        while (active == true) {
            ...
            if (currentTime > owner.propTimePeriod) {
                ...
                case ACTION_GET_CALIBVALUES:
                    // wait for eye values
                    System.out.println("Waiting for stable eye coordinates, XError= " + (xCalMax - xCalMin)
                            + ", YError = " + (yCalMax - yCalMin));
                    break;
                }
    ...
    public void captureCalibrationValues(int eyeX, int eyeY) {
        ...
        if (calEntries < CAL_ARRAY_SIZE - 1) {
            calEntries++;
        } else {
            calEntries = 0;
            ...
            for (int i = 0; i < CAL_ARRAY_SIZE; i++) {
                xAvg += calValuesX[i];
                yAvg += calValuesY[i];
                if (calValuesX[i] < xCalMin) {
                    xCalMin = calValuesX[i];
                }
                if (calValuesX[i] > xCalMax) {
                    xCalMax = calValuesX[i];
                }
                if (calValuesY[i] < yCalMin) {
                    yCalMin = calValuesY[i];
                }
                if (calValuesY[i] > yCalMax) {
                    yCalMax = calValuesY[i];
                }
            ...
            if ((xCalMax - xCalMin < MAX_CALIB_ERROR) && (yCalMax - yCalMin < MAX_CALIB_ERROR)) {
                owner.state = owner.STATE_WAIT_FOR_NEXT_CALIBPOINT; // point
                                                                    // done:
                                                                    // stop
                                                                    // feeding
                                                                    // eye
                ...
                for (int i = 0; i < calibCol; i++) {
                    if (calibMatrix[i][calibRow].eyeX > xAvg) {
                        calibError = 1;
                    }
                }
                for (int i = 0; i < calibRow; i++) {
                    if (calibMatrix[calibCol][i].eyeY > yAvg) {
                        calibError = 1;
                    }
                }
    ...
    public void getCalibratedLocations(int xe, int ye) {
        ...
        if (counter == 15) {
            System.out.println("\nxe/ye: " + xe + "/" + ye);
            System.out.println("LeftUp:" + ln + "/" + un + "  RightUp:" + rn + "/" + un + "  LeftDown:" + ln + "/" + dn
                    + "  RightDown:" + rn + "/" + dn);
        }
        ...
        if ((ln != rn) && (un != dn)) {
            ...
            if (counter == 15) {
                System.out
                        .println("x/yLocation:" + calibMatrix[ln][dn].xLocation + "/" + calibMatrix[ln][dn].yLocation);
            ...
            {
                if (counter == 15) {
                    System.out.println("vertical bordercase");
                }
            ...
            {
                if (counter == 15) {
                    System.out.println("horizontal bordercase");
                }
    ...
    public void getCalibratedLocationsWithPOSIT(int xe, int ye) {
        ...
        /*
         * ln=xIndex-1; if (ln<0) ln=0; rn=xIndex; if (rn>calibColumns)
         * rn=calibColumns; un=yIndex-1; if (un<0) un=0; dn=yIndex; if
         * (dn>calibRows) dn=calibRows;
         */
        ...
        /*
         * } else { if ((ln==rn) && (dn==un)) // corner point { if (counter==15)
         * System.out.println("cornercase"); actX=calibMatrix[ln][un].xLocation;
         * actY=calibMatrix[ln][un].yLocation;
         * 
         * uncompensatedX = actX; uncompensatedY = actY;
         * actX-=(currentpose.tvecx-owner.positObj.calibTRVecMatrix[ln][un].
         * tvecx)*owner.positObj.getDpmm();
         * actY-=(currentpose.tvecy-owner.positObj.calibTRVecMatrix[ln][un].
         * tvecy)*owner.positObj.getDpmm(); } else if (ln==rn) // point is on
         * vertical line { if (counter==15) System.out.println(
         * "vertical bordercase");
         * 
         * y1=calibMatrix[ln][un].eyeY; y2=calibMatrix[ln][dn].eyeY;
         * 
         * f1=(double)(ye-y1)/(double)(y2-y1);
         * 
         * actX=calibMatrix[ln][un].xLocation; //+f2*deltaX);
         ...
         * tvecx)*owner.positObj.getDpmm();
         * actY-=(currentpose.tvecy-owner.positObj.calibTRVecMatrix[ln][un].
         * tvecy)*owner.positObj.getDpmm(); } else // point is on horizontal
         * line { if (counter==15) System.out.println("horizontal bordercase");
         * 
         * x1=calibMatrix[ln][un].eyeX; x2=calibMatrix[rn][un].eyeX;
         * 
         * f1=(double)(xe-x1)/(double)(x2-x1);
         * 
         * actX=(double)calibMatrix[ln][un].xLocation+f1*(double)calibXStep;
