...
/**
 * PlatformLCDInstance represents the display found in the core CIM's 
 * hardware. It allows to activate and deactivate the display and to control
 * the brightness level. Moreover it encapsulates the menu handling abilities 
 * for the display in it interacting with the navigation buttons found on 
 * the core CIM. 
 * The class is implemented as an AbstractRuntimeComponentInstance as although
 * it does not have any ports or event ports because the use the components are
 * started after the CIM service which could not be guaranteed if the handling
 * of the display were implemented at the bundle level like CIM communication.
 * 
 * @author Christoph Weiss [weissch@technikum-wien.at]
 *         Date: Mar 7, 2011
 *         Time: 10:55:05 AM
 */
public class PlatformLCDInstance extends AbstractRuntimeComponentInstance 
...
{ 
	...
	private final byte [] textFontData   = { 0 };
	private final byte [] buttonEventMask   = { 0x1f };
    ...
    final char [] astericsLogoCharStream = {
    	 50, 0, 0, 0, 32, 0, 32, 0,
    	 0xF8, 0x7, 0xFF, 0xFF,
    	 0xF8, 0x3, 0xFF, 0xD7,
    	 0xFC, 0x1, 0xFF, 0xBB,
...
//    	port = CIMPortManager.getInstance().getConnection(CORE_CIM_ID);
    	if (port != null)
    	{
    		AstericsErrorHandling.instance.reportInfo(this, "Core CIM found");
	    	port.addEventListener(this);
	    	
	    	astericsLogoByteStream = new byte[astericsLogoCharStream.length];
	    	for (int i = 0; i < astericsLogoByteStream.length; i++)
	    	{
	    		astericsLogoByteStream[i] = (byte) astericsLogoCharStream[i];
	    	}
	    	
	    	CIMPortManager.getInstance().sendPacket(port, buttonEventMask, CORE_FEATURE_BUTTON_EVENT_MASK, 
	    	...
	    	CIMPortManager.getInstance().sendPacket(port, textFontData, CORE_FEATURE_DISPLAY_SET_TEXT_FONT, 
				      CIMProtocolPacket.COMMAND_REQUEST_WRITE_FEATURE, false);
			
			writeToDisplay("Framework started", true);
			displayActivity();
			...
			AstericsThreadPool.instance.execute(new Runnable()
            {
                ...
                public void run()
                {
                	while (true)
                	{
                    	try
                    	{
                    		if (System.currentTimeMillis() - lastActivity > DISPLAY_ACTIVITY_TIMEOUT)
                    		{
                    			displayInactive();
                    			menuHandler.inactivity();
                    		}
                    		Thread.sleep(1000);
                    	}
                    	catch (InterruptedException e)
    		...
    		AstericsErrorHandling.instance.reportInfo(this, "Could not find core CIM");
*/			
    }
   
   /**
    * Should be called to indicate activity on the display. This is used if
    * the display will reduce its brightness after a certain amount of
    * inactivity
    */
   ...
   private void displayInactive()
   {
	   ...
	   {
		   	byte [] data = { 0 };
//			port.sendPacket(astericsLogoByteStream, CORE_FEATURE_DISPLAY_BITMAP, 
//				      CIMProtocolPacket.COMMAND_REQUEST_WRITE_FEATURE, false);
		   	CIMPortManager.getInstance().sendPacket(port, data, CORE_FEATURE_DISPLAY_LIGHT, 
				      CIMProtocolPacket.COMMAND_REQUEST_WRITE_FEATURE, false);
		   	CIMPortManager.getInstance().sendPacket(port, null, CORE_FEATURE_DISPLAY_CLEAR, 
   ...
   public IRuntimeInputPort getInputPort(String portID)
     ...
     * otherwise.
     */
    private final IRuntimeInputPort displayStringInputPort
     ...
     * @param justified if true text will be centered on the display
     */
    public void writeToDisplay(String text, boolean justified)
    {
		...
		CIMPortManager.getInstance().sendPacket(port, textWindowData, CORE_FEATURE_DISPLAY_SET_TEXT_WINDOW, 
  		...
  		CIMPortManager.getInstance().sendPacket(port, buf.toString().getBytes(), CORE_FEATURE_DISPLAY_PRINT, CIMProtocolPacket.COMMAND_REQUEST_WRITE_FEATURE, false);
    }
    ...
    public void handleButton(int i)
    {
    	...
    	switch (i)
    	{
    	case 0: handleLeftPress();	break;
    	case 1: handleRightPress();	break;
    	case 2: handleDownPress();	break;
    	case 3: handleUpPress();	break;
    	case 4: handleOkPress();	break;
    	default: break;
    	}
    }
