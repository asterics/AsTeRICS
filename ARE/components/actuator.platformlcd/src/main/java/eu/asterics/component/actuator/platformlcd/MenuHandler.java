...
/**
 * MenuHandler operates all the navigation actions in the menu of the core
 * CIM display.
...
//	MenuItem dummyMenuItem 			= null;
	 ...
	 * 
	 * @param owner the display instance which generates this MenuHandler 
	 * instance 
	 */
...
//		dummyMenuItem.setRight(modelSwitchMenuItem);
		...
		for (int i = 0; i < storedModels.length; i++)
		{
			try
			{
				storedModels[i].left  = storedModels[ (((i - 1) % storedModels.length) + storedModels.length) % storedModels.length];
			...
			{
				e.printStackTrace();
				System.out.println(String.format("i = %d, modulo: %d", i, (((i - 1) % storedModels.length) + storedModels.length) % storedModels.length));
			}
		}
