...
/**
 * Implements the Graphical User Interface for the Dotmeter actuator plugin
 * 
 * @author Chris Veigl [veigl@technikum-wien.at] Date: Oct 7, 2012 Time:
 *         10:55:05 AM
 */
public class GUI extends JPanel {
    ...
    private void design(int width, int height) {
        ...
        dotPanel.addHierarchyListener(new HierarchyListener() {
            ...
            public void hierarchyChanged(HierarchyEvent e) {
                if ((HierarchyEvent.SHOWING_CHANGED & e.getChangeFlags()) != 0 && dotPanel.isShowing()) {
                    ...
                    paintPanel.repaint(dotPanel.getX(), dotPanel.getY(), dotPanelSize.width, dotPanelSize.height);
                    dotPanel.repaint();
                    dotPanel.revalidate();
    ...
    class PanelWithDotGraph extends JPanel {
        ...
        public void paintComponent(Graphics g) {
            ...
            if (owner.propDisplayDot == true) {
                ...
                g.fillOval((int) (x + actXPos - owner.propDotSize / 2), (int) (y + actYPos - owner.propDotSize / 2),
                        owner.propDotSize, owner.propDotSize);
            } else {
                g.drawOval((int) (x + actXPos - owner.propDotSize / 2), (int) (y + actYPos - owner.propDotSize / 2),
                        owner.propDotSize, owner.propDotSize);
            }
    ...
    void updateInput(final double xValue, final double yValue) {
        ...
        if (owner.propMode == owner.MODE_CLIPMINMAX) {
            if (drawXValue > actXMax) {
                drawXValue = actXMax;
            }
            if (drawXValue < actXMin) {
                drawXValue = actXMin;
            }
            if (drawYValue > actYMax) {
                drawYValue = actYMax;
            }
            if (drawYValue < actYMin) {
                drawYValue = actYMin;
            }
