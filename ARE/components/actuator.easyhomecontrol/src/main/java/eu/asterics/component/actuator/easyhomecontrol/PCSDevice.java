...
public class PCSDevice {
    ...
    public boolean send(int housecode, int addr, int command) {
        /*
         * byte [] buf = new byte[11]; buf[0] = 0x01; // hid report id buf[1] =
         * 0x06; // byte anzahl buf[2] = (byte) 0xF1; // Befehl ID byte [] hc =
         * FS20Utils.housecodeToHex(housecode); buf[3] = hc[0]; // 1111 HC
         * buf[4] = hc[1]; // 1111 HC buf[5] = FS20Utils.addressToHex(addr); //
         * 1111 Adresse buf[6] = (byte) command; // Befehl buf[7] = 0x00; //
         * Erweiterung try { if (dev != null) dev.write(buf); } catch
        ...
        try {
            /*
             * byte[] buffer = new byte[36]; int countera=0; for(int k =
             * 0;k<4;k++){ buffer[countera]=0; for(int j = 1;j<9;j++) {
             * buffer[j+countera]=rfCmdBuf[j+countera-(k+1)]; } countera =
             * countera + 9; }
             * 
             * 
             * 
