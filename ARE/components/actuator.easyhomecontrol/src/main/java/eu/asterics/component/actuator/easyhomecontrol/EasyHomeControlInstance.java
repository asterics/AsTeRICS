...
public class EasyHomeControlInstance extends AbstractRuntimeComponentInstance {
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                ...
                if (!pcs.open()) {
                    ...
                    AstericsErrorHandling.instance.reportError(EasyHomeControlInstance.this,
                            "Could not open/find EasyHome PCS Device. Please verify that the EasyHome Transceiver is connected to a USB port.");
                    pcs = null;
                    return;
                }
    ...
    private void closeDevice() {
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                ...
                if (pcs != null) {
                    if (!pcs.close()) {
                        AstericsErrorHandling.instance.reportInfo(EasyHomeControlInstance.this,
    ...
    private void sendDataToEasyHome(final int houseCode, final int addr, final int command) {
        ...
        try {
            ...
            logger.warning("Could not execute sendDataEasyHome in ModelExecutor thread: " + e.getMessage());
        }
    }
}