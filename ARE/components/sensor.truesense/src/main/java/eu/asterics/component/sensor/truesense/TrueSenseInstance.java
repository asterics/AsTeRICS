...
public class TrueSenseInstance extends AbstractRuntimeComponentInstance {
    ...
    byte[] turn_on = { 0x33, 0x33, 0x00, 0x02, 0x20, 0x23, 0x00,
            0x43 };/*
                    * Sync 2Byte; PayloadLength 2Byte; Payload 2Byte; CheckSum
                    * 2Byte;
                    */ // alt;{0x33,0x33,0x03,0x00,0x10,0x00,0x01,0x00,0x11 }
    byte[] request_data = { 0x33, 0x33, 0x00, 0x02, 0x10, 0x00, 0x00,
            0x10 };/*
                    * Sync 2Byte; PayloadLength 2Byte; Payload 2Byte; CheckSum
                    * 2Byte;
                    */ // alt;{0x33,0x33,0x03,0x00,0x10,0x00,0x01,0x00,0x11 }
    // |Sync 2Byte|PL_Length 2byte||Datacode 2Bytee|SubDdataCode
    // 1Byte|PayloadLength 1Byte|Checksum 2Byte 0x10+0x01 = 0x11
    ...
    public void handlePacketReceived(byte data) {
        ...
        // Sub-DataCode | TimeStamp | PDN | Misc. | ADC | Temp. | AccX | AccY |
        // AccZ | ED
        ...
        case 0:
            ...
            if (data == 0x33) { // System.out.println("Got 0x33 " +data);
                state = 1;
            }
            break;
...

        {
            ...
            payloadlength += (data & 0xFF);// 145 -> -110// -110 + 255 = 146
                                           // ->0x92
            payloadlength--;
            state = 4;
            break;
        ...
        case 5:
            ...
            // System.out.println("runningsum: "+runningsum+" Databuff:
            // "+data_buff[actpos] +" actpos: " +actpos);
            ...
            if (actpos == payloadlength) {
                state = 6;
                // System.out.println("actpos: " +actpos + "payloadlength:
                // "+payloadlength + "runningsum_final: "+runningsum);
            }

            break;

        case 6:
            // System.out.println("State 6: checksum1: " +checksum+ " data1: "
            // +data);
            checksum = (data & 0xff);
            // checksum=(data);
            // System.out.println("State 6: checksum2: " +checksum+ " data2: "
            // +data);
            checksum = checksum << 8;
            // System.out.println("State 6: checksum3: " +checksum+ " data3: "
            // +data);
            state = 7;
            break;
        ...
        case 7:
            ...
            // System.out.println("State 7: checksum3: " +checksum+ " data3: "
            // +data+ " runningsum3: "+runningsum );
            checksum += (data & 0xff);
            checksum--;
            ...
            for (i = 0; i < 64; i++) {
                if (((data_buff[9 + i * 2] & 128) == 128)) {
                    ...
                    d = ((((int) data_buff[9 + i * 2]) & 0xff) << 8) | (((int) data_buff[10 + i * 2]) & 0xfc);
                }
                ...
                public void run() {
                    ...
                    while (running) {
                        ...
                        try {
                            ...
                            // Thread.sleep(100);
                            CIMPortManager.getInstance().sendPacket(portController, request_data, (short) 0, (short) 0,
                                    false);
                            Thread.sleep(30);
    ...
    public void resume() {
        ...
        if (portController == null) {
            AstericsErrorHandling.instance.reportError(this,
                    "Could not construct Truesense raw port controller, please make sure that the COM port is valid.");
        }
        readerThread.start();
        super.resume();
    }
