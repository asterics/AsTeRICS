...
public class SerialPortInstance extends AbstractRuntimeComponentInstance 
{
    ...
    */
			...
			if (out != null)
			{
	          	try {
	        		...
	        		switch (propSendStringTerminator) {
		        		case 0: break;
		        		case 1:	out.write(13); break;
		        		case 2: out.write(10); break;
		        		case 3: out.write(13); out.write(10); break;
		        		case 4: out.write(0); break;	        			
	        		}
	        	} catch (Exception e) {};
    ...
    public void handlePacketReceived(byte actbyte)
	{
		...
		// System.out.println("received " + Integer.toHexString(0x000000ff & actbyte) +" hex");

		switch (propSendStringTerminator) {
			case 0: received += (char)actbyte; finished=true; break;
			case 1:	if (actbyte !=13) received += (char)actbyte; else finished=true; break;
			case 2: if (actbyte !=10) received += (char)actbyte; else finished=true; break;
			case 3: if ((actbyte !=13) && (actbyte != 10))
					{
					 endflag=false;
			        ...
			        {
			        	if (actbyte ==13) endflag=true; 
			            if ((endflag==true) && (actbyte == 10))
			            	   { endflag=false; finished=true; }
			        } 
			        break;
			case 4: if (actbyte !=0)  received += (char)actbyte; else finished=true; break;	        			
		}
		
		if (finished)
		{
		   opReceived.sendData(ConversionUtils.stringToBytes(received));
		   received=""; endflag=false;
		}
      ...
      */
      ...
      public void resume()
      {
    	portController = CIMPortManager.getInstance().getRawConnection(propComPort, propBaudRate,true);
  		if (portController == null) {
  			AstericsErrorHandling.instance.reportError(this, "Could not construct SerialPort controller, please make sure that the COM port is valid.");
  		}
  		readerThread.start();
  		super.resume();
      }
