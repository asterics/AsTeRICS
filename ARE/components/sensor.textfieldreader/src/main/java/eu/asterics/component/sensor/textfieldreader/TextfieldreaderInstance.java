...
/**
 * Implements the textfieldreader plugin, which compares the text input for an
 * edit field to up to seven command strings. On a string match, a corresponding
 * event is raised.
 * 
...
public class TextfieldreaderInstance extends AbstractRuntimeComponentInstance {
    ...
    public void processKeyboardInput(char actchar, int keycode) {
        ...
        act_string += actchar;
        // Logger.getAnonymousLogger().info(String.format("received: %c,
        // keybuffer:%s", actchar, act_string)));
        ...
        for (int i = 0; i < NUMBER_OF_COMMANDS; i++) {
            if (act_string.length() >= commands[i].length()) {
                if (commands[i].equalsIgnoreCase(act_string.substring(act_string.length() - commands[i].length()))) {
                    etpCommandRecognized[i].raiseEvent();
                    gui.logCommand(commands[i]);
                    // Logger.getAnonymousLogger().info(String.format("command:
                    // %d found, event raised!", i+1));
                }
            }
        }
