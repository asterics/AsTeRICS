...
public class GSMModemBridge implements Runnable
{
   ...
   * 
   * @param componentInstance owner of this object.
   * @param opRemotePhoneID SMS sender out port
   * @param opReceivedSMS SMS content out port
   * @param opErrorNumber Error out port
   * @param etpNewSMS new SMS available output event port
   * @param etpError error output event port
   */
  public GSMModemBridge(final IRuntimeComponentInstance componentInstance,
  final IRuntimeOutputPort opRemotePhoneID,
  final IRuntimeOutputPort opReceivedSMS,
  final IRuntimeOutputPort opErrorNumber,
  final IRuntimeEventTriggererPort etpNewSMS,
   ...
   * @param propSerialPort  serial port
   */
   ...
   * @param propSmsCenterID  SMS Center ID
   */
		  ...
		  if((phoneID.length()>0)&&(messageContent.length()>0))
		  {
			  ...
			  int result = sendSMS(phoneID,messageContent);
		  	  if(result<0)
		  	  {
		  		  ...
		  		  {
		  			  AstericsErrorHandling.instance.getLogger().warning("Modem Send SMS error: " +Integer.toString(result) + " Modem not ready !!!");
		  		  }
		  		  else
		  		  {
		  			  AstericsErrorHandling.instance.getLogger().warning("Modem Send SMS error: " +Integer.toString(result));
		  		  }
   ...
   *
   * @param port modem serial port
   * @param pin SIM card PIN
   * @param smsCenterID SMS center ID
   * @return error code
   */
  ...
  /**
   * This method is called back from the native code when there is found a new error.
   *
  ...
  private void error_callback(int code)
  {
	  if(code<0)
	  {
		  ...
		  {
			  AstericsErrorHandling.instance.getLogger().warning("Error:  " + Integer.toString(code) + " Modem is not ready !!!" );
		  }
		  else
		  {
