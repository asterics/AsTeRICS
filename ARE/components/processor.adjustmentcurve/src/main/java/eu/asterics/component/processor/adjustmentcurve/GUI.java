...
/**
 * Implements the Graphical User Interface for the AdjustmentCurve processor
 * plugin
 * 
...
public class GUI extends JPanel implements MouseListener, MouseMotionListener {
    ...
    class PanelWithDotGraph extends JPanel {
        ...
        public void paintComponent(Graphics g) {
            ...
            if (minDistance < 300) {
                ...
                g2d.fillRect(mx - 35, my - 20 - metrics.getHeight(), metrics.stringWidth(actCaption) + 10,
                        metrics.getHeight() + 5);
    ...
    void updateGraph(final double x, final double y) {
        ...
        if (owner.propMode == owner.MODE_CLIPMINMAX) {
            if (currentInputValue > actInMax) {
                currentInputValue = actInMax;
            }
            if (currentInputValue < actInMin) {
                currentInputValue = actInMin;
            }
        } else {
                ...
                if (answer != null) {
                    ...
                    if ((separator > 0) && (separator < answer.length())) {
                        ...
                        try {
                            ...
                            boolean valid = true;
                            if (activeNode > 0) {
                                if (owner.curvePoints.get(activeNode - 1).x >= xVal) {
                                    valid = false;
                                }
                            }
                            if (activeNode < owner.curvePoints.size() - 1) {
