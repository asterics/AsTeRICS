 ...
 *   Implements the AdjustmentCurve processor plugin, which transforms one
 *   signal curve into another one. The GUI of this element allows to draw
 *   the mapping from input to output signal
 *  
...
public class AdjustmentCurveInstance extends AbstractRuntimeComponentInstance
{
    ...
    public synchronized void load() {
        ...
        try{
            ...
            oIn.close();
            fIn.close();
            if (gui!=null) gui.updateMinMax();
        }
        catch(IOException e){
        	
        	AstericsErrorHandling.instance.getLogger().fine("Could not load adjustmentCurve file data/processor.adjustmentcurve/"+propFilename+".ser");
        	//AstericsErrorHandling.instance.getLogger().fine("Using default values ...");
    ...
    public synchronized void save() {
        ...
        try{
        	...
        	AstericsErrorHandling.instance.getLogger().fine("Could not save adjustmentCurve file data/processor.adjustmentcurve/"+propFilename+".ser");
            // e.printStackTrace();
        }
        finally {
            try {
    ...
    */
     ...
     */
    private final IRuntimeInputPort ipCurveName
            = new DefaultRuntimeInputPort()
    {
        public void receiveData(byte[] data)
        {
