...
/**
 * 
 * This module allows to configure and start/stop the stimulation of the
 * Flickering Light Panels.
 * 
 * 
 * @author Laura Dubreuil Vall [laura.dubreuil@starlab.es] Date: 30/08/2012
 *         Time: 15:01
 */
public class FlickeringLightStimulatorInstance extends AbstractRuntimeComponentInstance implements CIMEventHandler {
    ...
    void setConfiguration() {
        ...
        send_data(propFrequencyPanel1, propDcPanel1, propIntPanel1, checksumPanel1); // Panel
                                                                                     // 1

        checksumPanel2 = calc_checksum(propFrequencyPanel2 * 2, propDcPanel2, propIntPanel2);
        send_data(propFrequencyPanel2, propDcPanel2, propIntPanel2, checksumPanel2); // Panel
                                                                                     // 2

        checksumPanel3 = calc_checksum(propFrequencyPanel3 * 2, propDcPanel3, propIntPanel3);
        send_data(propFrequencyPanel3, propDcPanel3, propIntPanel3, checksumPanel3); // Panel
                                                                                     // 3

        checksumPanel4 = calc_checksum(propFrequencyPanel4 * 2, propDcPanel4, propIntPanel4);
        send_data(propFrequencyPanel4, propDcPanel4, propIntPanel4, checksumPanel4); // Panel
                                                                                     // 4
    ...
    int calc_checksum(int data1, int data2, int data3) {
        ...
        for (i = 0x01, checksum = 0, n = 0; n < 8; i = i << 1, n++) // shift
                                                                    // data
        {
            if ((i & data1) == i) {
                checksum++;
    ...
    public void handlePacketError(CIMEvent e) {
        ...
        /*
         * CIMEventErrorPacketFault EventErrorPacketFault =
         * (CIMEventErrorPacketFault)e;
         * 
         * AstericsErrorHandling.instance.reportError(this,
         * EventErrorPacketFault.toString());
         * 
         * CIMEventErrorPacketLost EventErrorPacketLost =
         * (CIMEventErrorPacketLost)e;
         * 
         * AstericsErrorHandling.instance.reportError(this,
         * EventErrorPacketLost.toString());
         */
    ...
    public void start() {
        ...
        byte c = 0x00;
        port = CIMPortManager.getInstance().getRawConnection(propComPort, 9600); // COM
                                                                                 // Port
                                                                                 // and
                                                                                 // baud
                                                                                 // rate
        ...
        if (port != null) {
             ...
             * CIMPortManager.getInstance().sendPacket(port, idle, trash, trash,
             * false); CIMPortManager.getInstance().sendPacket(port, end, trash,
             * trash, false);
             */

            // statusRequest();
            /*
             * setConfiguration(); startStimulation(); try { Thread.sleep(5000);
            ...
            /*
             * byte[] status_request =
             * ConversionUtils.stringToBytes("Status_Request");
             * 
             * CIMPortManager.getInstance().sendPacket(port, status_request,
             * trash, trash, false);
             * CIMPortManager.getInstance().sendPacket(port, end, trash, trash,
             * false);
             */
        } else {
