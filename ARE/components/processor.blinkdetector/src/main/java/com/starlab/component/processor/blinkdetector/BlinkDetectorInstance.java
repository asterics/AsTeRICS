...
/**
 * 
 * This module implements the detection of the artefact a blink produces in the
 * EEG signal
 * 
...
public class BlinkDetectorInstance extends AbstractRuntimeComponentInstance {
    public native long nativeCreateBlinkDetector(int sampleRate, int maxThreshold, int minThreshold, int interval1,
            int interval2);
    ...
    public void start() {
        if (blinkDetectorHandle == 0) {
            blinkDetectorHandle = nativeCreateBlinkDetector(propSampleRate, propMaxThreshold, propMinThreshold,
                    propBlinkLength, propDoubleBlinkSeparation);
        }
        super.start();
    }
