 ...
 * 
 * 
 * @author Chris Veigl [veigl@technikum-wien.at] Date: 11.11.2011 Time: 00:07 AM
 */
public class ArduinoInstance extends AbstractRuntimeComponentInstance implements CIMEventHandler {
    ...
    private int pin3Mode = 0; // mode of pin 3 (0 -> no PWM, 1 -> servo, 2->
                              // 500Hz PWM)
    private int pin5Mode = 0; // mode of pin 5 (0 -> no PWM, 1 -> servo, 2->
                              // 500Hz PWM)
    private int pin6Mode = 0; // mode of pin 6 (0 -> no PWM, 1 -> servo, 2->
                              // 500Hz PWM)
        ...
        for (int i = 0; i < NUMBER_OF_PINS; i++) {
            ...
            if (s.equalsIgnoreCase(propertyName)) {
                ...
                case PINMODE_PWM_500Hz:
                    ...
                    break;
                /*
                 * case PINMODE_PWM_10kHz: if((i+STARTPIN) == 3) pin3Mode=3;
                 * if((i+STARTPIN) == 5) pin5Mode=3; if((i+STARTPIN) == 6)
                 * pin6Mode=3; pinMask &= ~(1<<(i+STARTPIN)); break; case
                 * PINMODE_PWM_28kHz: if((i+STARTPIN) == 3) pin3Mode=4;
                 * if((i+STARTPIN) == 5) pin5Mode=4; if((i+STARTPIN) == 6)
                 * pin6Mode=4; pinMask &= ~(1<<(i+STARTPIN)); break;
                 */
                }
                if (pin3Mode != 0) {
        ...
        public void receiveData(byte[] data) {
            ...
            if (pin3Mode != 0) {
                AstericsErrorHandling.instance.reportDebugInfo(null,
                        String.valueOf(3 + (value << 8) + (pin3Mode << 4)) + "  Value 3");
    ...
    private final IRuntimeInputPort ipPwm5 = new DefaultRuntimeInputPort() {
        ...
        public void receiveData(byte[] data) {
            ...
            if (pin5Mode != 0) {
                AstericsErrorHandling.instance.reportDebugInfo(null,
                        String.valueOf(5 + (value << 8) + (pin5Mode << 4)) + "  Value 5");
                sendArduinoWriteFeature(ARDUINO_CIM_FEATURE_SET_PWM, 5 + (value << 8) + (pin5Mode << 4));// hier
                                                                                                         // PWM
            }
        }
    ...
    private final IRuntimeInputPort ipPwm6 = new DefaultRuntimeInputPort() {
        ...
        public void receiveData(byte[] data) {
            ...
            if (pin6Mode != 0) {
                AstericsErrorHandling.instance.reportDebugInfo(null,
                        String.valueOf(6 + (value << 8) + (pin6Mode << 4)) + "  Value 6");
                sendArduinoWriteFeature(ARDUINO_CIM_FEATURE_SET_PWM, 6 + (value << 8) + (pin6Mode << 4));// hier
                                                                                                         // PWM
            }
        }
    };
    ...
    /**
     * Handles an input packet from Arduino CIM. Reads the values of all ADC
            ...
            for (int i = 0; i < NUMBER_OF_PINS; i++) {
                if (((newPinValue & (1 << (i + STARTPIN))) != (currentPinValue & (1 << (i + STARTPIN))))
                    ...
                    if ((newPinValue & (1 << (i + STARTPIN))) != 0) {
                        ...
                        etpChangedHighToLow[i].raiseEvent();
                    }
                }
    ...
    public void handlePacketReceived(CIMEvent e) {
        ...
        case CIMProtocolPacket.COMMAND_EVENT_REPLY:
            ...
            if (featureAddress == ARDUINO_CIM_FEATURE_GET_PINVALUE) {
                // System.out.println ("Incoming Event: PINReport
                // "+(128+(int)packet.getSerialNumber()));
                handleArduinoPinReport(packet);
            }
            break;
        case CIMProtocolPacket.COMMAND_REPLY_WRITE_FEATURE:
             ...
             * System.out.println (" Set PinDirection."); } if (featureAddress
             * == ARDUINO_CIM_FEATURE_SET_PINSTATE) { System.out.println (
             * " Set PinState."); } if (featureAddress ==
             * ARDUINO_CIM_FEATURE_SET_PINMASK) { System.out.println (
             * " Set PinMask."); } if (featureAddress ==
             * ARDUINO_CIM_FEATURE_SET_ADCPERIOD) { System.out.println (
             * " Set AdcPeriod."); }
             */
            break;
        }
    ...
    public void start() {
        if (port == null) {
            ...
            // port =
            // CIMPortManager.getInstance().getConnection(ARDUINO_CIM_ID);
        }
        if (port != null) {
            ...
            firstPinReport = true;
            CIMPortManager.getInstance().sendPacket(port, null, ARDUINO_CIM_FEATURE_GET_PINVALUE,
                    CIMProtocolPacket.COMMAND_REQUEST_READ_FEATURE, false);
            CIMPortManager.getInstance().sendPacket(port, null, (short) 0, CIMProtocolPacket.COMMAND_REPLY_START_CIM,
                    false);
        } else {
            AstericsErrorHandling.instance.reportError(this, "Could not find Arduino (ID " + propUniqueID
                    + ") Please verify that the Arduino Module is connected to an USB Port and that the correct firmware is installed.");
        }
