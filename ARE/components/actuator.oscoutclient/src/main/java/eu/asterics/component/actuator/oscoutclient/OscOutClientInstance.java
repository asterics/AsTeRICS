    ...
    */
     ...
     */
     synchronized private void sendOutput()
     {
    	 ...
    	 //AstericsErrorHandling.instance.reportInfo(OscGestureInstance.instance, "Send_output");
    	
    	 bndlData	= new OSCBundle( System.currentTimeMillis() );
    	 bndlData.addPacket(new OSCMessage(OscOutClientInstance.instance.propAddressCh1, new Object[] { new Float (outCH1), new Float (outCH2),new Float (outCH3),new Float (outCH4)}));
    	 //bndlData.addPacket(new OSCMessage(OscOutClientInstance.instance.propAddressCh1, new Object[] {  new Integer (5)}));
    	 try {
	...
	private final IRuntimeInputPort ipCH1  = new DefaultRuntimeInputPort()
	{
		...
		{	
			...
			outchannel1 = (float)value;
			//float floatValue;
			
			//AstericsErrorHandling.instance.reportInfo(OscOutClientInstance.instance, String.valueOf(OscOutClientInstance.instance.outchannel1));
			try {
				//floatValue= (float)value;
				OscOutClientInstance.instance.c.send( new OSCMessage( "/channel1", new Object[] { new Float(outchannel1) }));
	  		}
...
//			
	...
	private final IRuntimeInputPort ipStringIN  = new DefaultRuntimeInputPort()
	{
		public void receiveData(byte[] data)
		{
			...
			bndlData	= new OSCBundle( System.currentTimeMillis() );
	    	 bndlData.addPacket(new OSCMessage(OscOutClientInstance.instance.propAddressStringCh, new Object[] { new String (text)}));
	    	 try {
	    		 OscOutClientInstance.instance.c.send( bndlData );
	    	 }
	    	 catch( IOException e2 ) {
      ...
      */
      ...
      public static void client( String protocol )
  		...
  		try {
  			...
  			OscOutClientInstance.instance.c.setTarget( new InetSocketAddress( InetAddress.getByName(OscOutClientInstance.instance.propPeerAddress), OscOutClientInstance.instance.propPort ));
  			//postln( "  start()" );
  			OscOutClientInstance.instance.c.start();
  		}
  		catch( IOException e1 ) {
  		...
  		//OscOutClientInstance.instance.c.dumpOSC( OSCChannel.kDumpBoth, System.err );
