...
package eu.asterics.component.processor.stringpathselector;
...
/**
 *   Implements the string pathselector plugin, which selects routes the input
 *   of one input port to one out of 4 output ports. The selection can be 
 *   done via event ports.
 *   
 *  
 ...
 *         Time: 04:55:00 PM
 */
public class StringPathselectorInstance extends AbstractRuntimeComponentInstance
{
	 ...
	 */
	public StringPathselectorInstance()
	{
	...
	private final IRuntimeInputPort ipIn	= new DefaultRuntimeInputPort()
	{
		public void receiveData(byte[] data)
		{
			...
			switch (selectedPort) {
			case 1: opOut1.sendData(ConversionUtils.stringToBytes(in));
			break;
			case 2: opOut2.sendData(ConversionUtils.stringToBytes(in));
			break;
			case 3: opOut3.sendData(ConversionUtils.stringToBytes(in));
			break;
			case 4: opOut4.sendData(ConversionUtils.stringToBytes(in));
			break;
			default: break;
			}
		}
	...
	final IRuntimeEventListenerPort elpSelectNext 	= new IRuntimeEventListenerPort()
	{
		...
		{
			selectedPort++; 
			AstericsErrorHandling.instance.getLogger().fine("Selected:"+selectedPort);
			if (selectedPort>propActivePorts) selectedPort=1;
		}
	}; 
	...
	final IRuntimeEventListenerPort elpSelectPrev 	= new IRuntimeEventListenerPort()
	{
		...
		{
			selectedPort--; 
			if (selectedPort<1)  selectedPort=propActivePorts;
		}
	}; 
