...
/**
 * Implements the string pathselector plugin, which selects routes the input of
 * one input port to one out of 4 output ports. The selection can be done via
 * event ports.
 * 
 * 
...
public class StringPathselectorInstance extends AbstractRuntimeComponentInstance {
    ...
    private final IRuntimeInputPort ipIn = new DefaultRuntimeInputPort() {
        ...
        public void receiveData(byte[] data) {
            ...
            switch (selectedPort) {
            case 1:
                opOut1.sendData(ConversionUtils.stringToBytes(in));
                break;
            case 2:
                opOut2.sendData(ConversionUtils.stringToBytes(in));
                break;
            case 3:
                opOut3.sendData(ConversionUtils.stringToBytes(in));
                break;
            case 4:
