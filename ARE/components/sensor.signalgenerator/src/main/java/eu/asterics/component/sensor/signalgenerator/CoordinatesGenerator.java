...
public class CoordinatesGenerator implements Runnable {
     ...
     */
    public CoordinatesGenerator(final SignalGeneratorInstance.OutputPort out, final int sendInterval,
            final int waveForm, final double frequency, final double amplitude, final double phaseShift,
            final double offset) {
            ...
            if (active == 1) {
                while (System.currentTimeMillis() - starttime > count) {
                    ...
                    case WAVEFORM_RANDOM:
                        out.sendData(offset + r.nextDouble() * amplitude);
                        break;
                    case WAVEFORM_SINE:
                        out.sendData(offset
                                + amplitude * Math.sin(((double) count + phaseShift) / 1000 * frequency * 2 * Math.PI));
                        break;
                    case WAVEFORM_SAWTOOTH:
                        out.sendData(offset + ((int) ((count + phaseShift) * frequency) % 1000) * amplitude / 1000);
                        break;
                    case WAVEFORM_RECTANGLE:
                        if (count >= (int) (500 * rstep / frequency)) {
                            rstep++;
                        }
