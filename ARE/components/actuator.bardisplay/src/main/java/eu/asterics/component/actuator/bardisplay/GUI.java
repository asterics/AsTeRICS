...
public class GUI extends JPanel
{
     ...
     * @param owner    the Bardisplay instance
     */
...
//	    add(barCaption, BorderLayout.SOUTH);
		...
		barPanel.addHierarchyListener(new HierarchyListener() {
			...
			public void hierarchyChanged(HierarchyEvent e) 
			{
						...
						&& barPanel.isShowing()) 
				{
					...
					paintPanel.repaint(barPanel.getX(), 
							barPanel.getY(), 
							barPanelSize.width, barPanelSize.height);
					barPanel.repaint();
					barPanel.revalidate();
     ...
     class PanelWithBarGraph extends JPanel {
        ...
        public void paintComponent(Graphics g) {
            ...
            if (owner.propDisplayThreshold)
            {
                ...
                g.drawRect(x, (int)(y+h
                    -(owner.propThreshold-actMin)*drawFactor),w-1,2);   	 
            }
    ...
    void updateInput(final double value)
    {
    	...
    	if (inputCount > owner.propDisplayBuffer) 
    	{
        	...
        	{
        	   if (drawValue>actMax) drawValue= actMax;
        	   if (drawValue<actMin) drawValue= actMin;
        	}
