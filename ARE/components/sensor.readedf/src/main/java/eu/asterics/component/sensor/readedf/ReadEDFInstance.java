 ...
 * 
 * Have a look at "http://www.edfplus.info/specs/edf.html" for full edf-file
 * specification
 * 
...
public class ReadEDFInstance extends AbstractRuntimeComponentInstance {
    ...
    int numberOfSignals = 32; // number of recorded signals in the file (number
                              // will be replaced after the file has been read)
    ...
    float periodDuration[] = new float[numberOfSignals]; // period duration for
                                                         // the samples of each
                                                         // signal
    ...
    /**
     * reads the samples from a signal, defined by signalNumberParamter,
     * converts them and sends them to the corresponding output port
     * 
     * @param number
     *            of the output port ( through the constructor)
     */
    public class signalTransmitClass extends Thread {
        ...
        public signalTransmitClass(int signalNumberParameter) {
            this.signalNumber = signalNumberParameter; // to declare for which
                                                       // signal this object is
                                                       // used for
        }
        ...
        public void run() {
            ...
            try {
                filePointerInstance = new RandomAccessFile(propFileName, "r");
            } catch (IOException e) {
                AstericsErrorHandling.instance.getLogger()
                        .severe("Error reading file (at run method): " + propFileName);
            }

            for (int i = 0; i < signalNumber; i++) {
                signalSamplesOffset += (samplesPerRecord[i] * 2); // to
                                                                  // determine
                                                                  // the offset
                                                                  // of each
                                                                  // signal-start
            ...
            for (long recordNumber = 0; (recordNumber < numberOfDataRecords) && samplingShouldRun; recordNumber++) {
                for (long sampleNumber = 0; (sampleNumber < samplesPerRecord[signalNumber])
                        && samplingShouldRun; sampleNumber++) {
                    try {
                        filePointerInstance.seek(
                                headerSize + (recordSize * recordNumber) + signalSamplesOffset + (2 * sampleNumber));
                    } // go to the first position of the signal records in the
                      // file
                    catch (IOException e) {
                        AstericsErrorHandling.instance.getLogger()
                                .severe("Could not read from file for signal number: " + signalNumber);
                    }
                    ;
                    // ===================================================================================================
                    // read the bytes, representing a sample, from file and
                    // merge them
                    // ===================================================================================================
                    try {
                        sample = filePointerInstance.read();
                        sample += (filePointerInstance.read() * 256); // read
                                                                      // the
                                                                      // next
                                                                      // byte
                                                                      // and
                    ...
                    double physicalSample = (((double) sample - digitalMin[signalNumber])
                            / (digitalMax[signalNumber] - digitalMin[signalNumber])
                            * (physicalMax[signalNumber] - physicalMin[signalNumber]) + physicalMin[signalNumber]);
                    ...
                    if (timeUntilNextSample > 0) {
                        ...
                        } // suspend the thread, for the right sampling
                          // frequency, and allow the other threads to send
                          // their data
                    ...
                    opCH[signalNumber].sendData(physicalSample); // send the
                                                                 // sample to
                                                                 // the
                                                                 // corresponding
                                                                 // output port
            ...
            if (filePointer != null) {
                try {
                    ...
                    AstericsErrorHandling.instance.getLogger().severe("Error closing file: " + propFileName);
                }
            }
            samplingRuns = false;
        }
     ...
     * 
     * @param position
     *            = position of the data in the file
     * @param numberOfBytes
    ...
    float readPropertyFromEDFHeader(int position, int numberOfBytes) {
        ...
        float value = 0;
        int stateMachine = 0; // (state 0: no "." or "E" or "e" where found so
                              // far) / (state 1: "." was found ) / ( state 2:
                              // "E" or "e" was found )
        int decimalPosition = 1;
        int power = 0;
        byte[] asciiValue = new byte[numberOfBytes]; // for the characters in
                                                     // the file representing a
                                                     // number
        try {
            filePointer.seek(position);
            filePointer.read(asciiValue); // read "numberOfBytes" Bytes
        } catch (IOException e) {
            AstericsErrorHandling.instance.reportInfo(this, "Error reading file: " + propFileName);
        }
        // ===================================================================================================
        // convert each of the ASCII-chars to a digit and merge them into a
        // number
        // ===================================================================================================
    ...
    public void start() {
        ...
        try {
            filePointer = new RandomAccessFile(propFileName, "r");
        } catch (IOException e) {
        ...
        // ===================================================================================================
        numberOfDataRecords = (int) readPropertyFromEDFHeader(236, 8); // get
                                                                       // the
                                                                       // number
                                                                       // of
                                                                       // records,
                                                                       ...
                                                                       // caputured
                                                                       // sequentially
        numberOfSignals = (int) readPropertyFromEDFHeader(252, 4); // get the
                                                                   // number of
                                                                   // signals
        headerSize = 256 * (1 + numberOfSignals); // get the header size
        final int numOfSamplesPosition = headerSize - numberOfSignals * (32 + 8); // get
                                                                                  // the
                                                                                  // position,
                                                                                  // where
                                                                                  // the
                                                                                  ...
                                                                                  // is
                                                                                  // located
        final float durationOfDataRecord = readPropertyFromEDFHeader(244, 8); // get
                                                                              // the
                                                                              // duration
                                                                              // of
                                                                              // a
                                                                              ...
                                                                              // record
        // ===================================================================================================
        // Getting the data of the second header for the signal depending values
        // (each signal has different attributes)
        // ===================================================================================================
        for (int signalNumber = 0; signalNumber < numberOfSignals; signalNumber++) {
            samplesPerRecord[signalNumber] = (int) readPropertyFromEDFHeader(numOfSamplesPosition + signalNumber * 8,
                    8); // get the "number of samples" of each signal
            recordSize += (samplesPerRecord[signalNumber] * 2); // get the
                                                                // number of
                                                                // samples of
                                                                // all signals
                                                                // in one record
                                                                ...
                                                                // consists of 2
                                                                // bytes)
            periodDuration[signalNumber] = durationOfDataRecord / samplesPerRecord[signalNumber] * 1000; // calculate
                                                                                                         // the
                                                                                                         // period
                                                                                                         // duration
                                                                                                         // [in
        ...
        // ===================================================================================================
        // Starting a thread for each signal. They will read the corresponding
        // samples and send them to their output ports
        // ===================================================================================================
    ...
    public void stop() {
        ...
        if (filePointer != null) {
            try {
                ...
                AstericsErrorHandling.instance.reportError(this, "Error closing file: " + propFileName);
            }
        }
        super.stop();
    }
