...
public class GameOverScreen extends AbstractScreen {
    ...
    public GameOverScreen(AstericsPong game) {
        super(game);
    }
    ...
    public void show() {
        super.show();
        /*
         * // retrieve the default table actor Table table = getTable();
         * table.add( "GAME OVER!" ).spaceBottom( 50 ); table.row(); Player pl =
         * (GameWorld.instance.players[0].lifes >
         * GameWorld.instance.players[1].lifes) ? GameWorld.instance.players[0]
         * : GameWorld.instance.players[1];
         * 
         * // register the button "start game" table.add( pl.name + " WINS!"
         * ).spaceBottom( 100 ); table.row(); table.add(""); for (Player p :
         * GameWorld.instance.players) { table.add(p.name); } table.row();
         * table.add("Score:"); for (Player p : GameWorld.instance.players) {
         * table.add("   " + p.score + "   "); } table.row();
         * table.add("Calories:"); for (Player p : GameWorld.instance.players) {
         * table.add("   " + PongGameProperties.eventsToCaloryMultiplier *
         * p.inputs + "   "); } table.row();
         */
    ...
    public void render(float delta) {
        ...
        if (PongGameProperties.eventsToCaloryMultiplier > 0) {
            ...
            pl = (p1Calories > p2Calories) ? GameWorld.instance.players[0] : GameWorld.instance.players[1];
            ...
            getFont().draw(getBatch(), " " + numOutput + " Kilo-Kalorien (kCal).", 390 * getScaleFactor(),
                    390 * getScaleFactor());
            numOutput = formatter.format((double) (p1Calories + p2Calories) * 0.000001163);
            getFont().draw(getBatch(), " " + numOutput + " Kilowatt-Stunden (kWh)", 390 * getScaleFactor(),
                    335 * getScaleFactor());

            getFont().draw(getBatch(), pl.name + " siegt nach Punkten ", 450 * getScaleFactor(),
                    630 * getScaleFactor());

        } else {
            getFont()
                    .draw(getBatch(),
                            pl.name + " siegt nach Punkten: " + GameWorld.instance.players[0].score + " vs. "
                                    + GameWorld.instance.players[1].score,
