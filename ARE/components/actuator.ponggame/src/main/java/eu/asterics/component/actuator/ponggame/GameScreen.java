        ...
        if (useDebugRenderer) {
            ...
            if (GameWorldStatic.instance != null) {
                ...
                if (GameWorldStatic.instance.boundVectors != null) {
                    ...
                    for (int i = 1; i < GameWorldStatic.instance.boundVectors.length; i++) {
                        shapeRenderer.line(GameWorldStatic.instance.boundVectors[i].x * PHYS_TO_REND,
            ...
            if (GameWorld.instance != null) {
                ...
                for (GameBall ball : GameWorld.instance.paddles) {
                    if (ball.type == 0) {
                        shapeRenderer.circle(ball.x * PHYS_TO_REND, ball.y * PHYS_TO_REND, ball.radius * PHYS_TO_REND);
                    } else if (ball.type == 1) {
                        shapeRenderer.rect((ball.x - ball.width) * PHYS_TO_REND, (ball.y - ball.radius) * PHYS_TO_REND,
                                ball.width * 2 * PHYS_TO_REND, ball.radius * 2 * PHYS_TO_REND);
                    }
                }

                shapeRenderer.circle(GameWorld.instance.ball.x * PHYS_TO_REND, GameWorld.instance.ball.y * PHYS_TO_REND,
                ...
                if (game.opBallY != null) {
                    game.opBallY.sendData(ConversionUtils.intToBytes((int) (GameWorld.instance.ball.y * PHYS_TO_REND)));
                }

                shapeRenderer.circle(GameWorld.instance.bumper.x * PHYS_TO_REND,
                        GameWorld.instance.bumper.y * PHYS_TO_REND, GameWorld.instance.bumper.radius * PHYS_TO_REND);

                int lifes = GameWorld.instance.players[0].lifes;
                for (int i = 0; i < Math.min(5, lifes); i++) {
                    shapeRenderer.circle(0.2f * PHYS_TO_REND,
                ...
                for (int i = 0; i < Math.min(5, lifes); i++) {
                    shapeRenderer.circle(9.3f * PHYS_TO_REND,
                            (2.0f + i * 3 * GameWorld.instance.ball.radius) * PHYS_TO_REND,
                            GameWorld.instance.ball.radius * PHYS_TO_REND);
                }
            ...
            for (Player p : GameWorld.instance.players) {
                getFont().draw(getBatch(), p.name + " : " + p.score,
                        (4.0f + 9 * p.index) * getScaleFactor() * PHYS_TO_REND,
                        0.80f * PHYS_TO_REND * getScaleFactor());

                // for (int i=0; i<p.lifes;i++)
                // drawImage(ballImage,(int)((1.5f + 9 * p.index + i*0.6f)*
                // PHYS_TO_REND) , (int)(0.50f * PHYS_TO_REND));
