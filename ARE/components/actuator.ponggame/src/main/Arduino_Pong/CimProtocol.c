...
void generate_ADCFrame()
{
	...
	c1+=encode_read1(); if (c1<0) c1=0; if (c1>300) c1=300;
	c2+=encode_read2(); if (c2<0) c2=0; if (c2>300) c2=300;

	CIM_frame.data[0]= (uint8_t)(c1&0xff);
	CIM_frame.data[1]= (uint8_t)(c1>>8);

	CIM_frame.data[2]= (uint8_t)(c2&0xff);
	CIM_frame.data[3]= (uint8_t)(c2>>8);
