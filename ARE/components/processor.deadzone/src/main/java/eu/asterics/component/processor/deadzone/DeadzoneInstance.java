...
/**
 *   Implements the DeadZone plugin, which can attenuate 1d or 2d signals
 *   which are inside or outside a defined radius around a center value 
 * 
...
public class DeadzoneInstance extends AbstractRuntimeComponentInstance
{
    ...
    /**
     * processes the input values according to the selected mode of the deadzone plugin:
     * for MODE_INNER, only values inside the radius are routed to the outputs
     * for MODE_OUTER, only values outside the radius are routed to the outputs
     * for MODE_DEADZONE, only values outside the radius are routed to the outputs, 
     * 					and the input values are modified to start with 0 at the outer zone
     */
