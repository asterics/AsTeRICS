...
public class SensorboardInstance extends AbstractRuntimeComponentInstance implements CIMEventHandler
{
	...
	private static final short SENSORBOARD_FEATURE_SERIAL_NUMBER = 0x0000;
    ...
    */
    ...
    public Object setRuntimePropertyValue(String propertyName, Object newValue)
    {
		if ("refreshInterval".equalsIgnoreCase(propertyName))
		{
			...
			if ( port != null)
			{
				CIMPortManager.getInstance().sendPacket(port, ConversionUtils.intToBytes(propRefreshInterval), SENSORBOARD_FEATURE_ACTIVATE_PERIODIC_VALUE, 
						CIMProtocolPacket.COMMAND_REQUEST_WRITE_FEATURE, false);
						//Logger.getAnonymousLogger().info("Sent SENSORBOARD activate periodic update");
			}			
			return oldValue;
		}
	...
	/**
	 * Handles an input packet from the SENSORBOARD CIM. Reads the values on all active 
	...
	private void handleSensorboardInputValuePacket(CIMProtocolPacket packet)
	{
		...
		//opAccX.sendData(ConversionUtils.intToBytes( (((short) b[0]) & 0xff) | ((((short) b[1]) & 0xff) << 8)));
		opAccX.sendData(ConversionUtils.intToBytes(byteToSignedInt(b[0],b[1])));
		
		//opAccY.sendData(ConversionUtils.intToBytes( (((short) b[2]) & 0xff) | ((((short) b[3]) & 0xff) << 8)));
		opAccY.sendData(ConversionUtils.intToBytes(byteToSignedInt(b[2],b[3])));

		//opAccZ.sendData(ConversionUtils.intToBytes( (((short) b[4]) & 0xff) | ((((short) b[5]) & 0xff) << 8)));
		...
		opPressure.sendData(ConversionUtils.intToBytes( (((int) b[34]) & 0xff))); // | ((((int) b[35]) & 0xff) << 8)));
