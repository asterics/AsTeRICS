...

{
    ...
    int inSlot0, inSlot1, inSlot2, inSlot3, inSlot4, inSlot5, inSlot6, inSlot7, inSlot8, inSlot9, inSlot10, inSlot11,
            inSlot12, inSlot13, inSlot14, inSlot15;// Usage of an event trigger
                                                   // port e.g.:
    ...
    synchronized private final void sendSlot(byte b) {
        byte[] data = { b };
        CIMPortManager.getInstance().sendPacket(portController, data, HID_FEATURE_JOYSTICK_UPDATE,
    ...
    public void start() {
        ...
        if (portController == null) {
            AstericsErrorHandling.instance.reportError(this,
                    "Could not construct raw port controller, please verify that the COM port number is valid.");
        } else {
            if (propDebug) {
