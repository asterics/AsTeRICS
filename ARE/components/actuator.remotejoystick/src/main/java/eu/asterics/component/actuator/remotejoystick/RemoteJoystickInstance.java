...
/**
 *   RemoteJoystickInstance interfaces with the HID actuator dongle 
 *   to emulate a joystick on a remote PC via USB HID 
 *  
...
public class RemoteJoystickInstance extends AbstractRuntimeComponentInstance
{
    ...
    */
	...
	synchronized private final void sendJoystickUpdate (boolean forceSend)
	{
				...
				b[11] =  (byte) ((joystickBtn & (1<<3)) == 0 ? 0 : 255);  // triangle axis
		...
		if (forceSend || (propRefreshInterval < 1) || (System.currentTimeMillis() - lastUpdate > propRefreshInterval))
		{
			if (forceSend) 
			{
				...
				if (forcedUpdates < MAX_FORCED_UPDATES) {
					if (port != null)
					{
						CIMPortManager.getInstance().sendPacket(port, b, HID_FEATURE_JOYSTICK_UPDATE, CIMProtocolPacket.COMMAND_REQUEST_WRITE_FEATURE, false);
					}
     ...
     */
	private final IRuntimeInputPort ipJoystickX
            = new DefaultRuntimeInputPort()
    {
        public void receiveData(byte[] data)
        {
            joystickX = ConversionUtils.byteArrayToInt(data);
            // Logger.getAnonymousLogger().info("received Joystick y data: " +  joystick_y);
     ...
     */
    private final IRuntimeInputPort ipJoystickY
            = new DefaultRuntimeInputPort()
    {
        public void receiveData(byte[] data)
        {
     ...
     */
	private final IRuntimeInputPort ipJoystickZ
            = new DefaultRuntimeInputPort()
    {
        public void receiveData(byte[] data)
        {
            joystickZ = ConversionUtils.byteArrayToInt(data);
            // Logger.getAnonymousLogger().info("received Joystick z data: " +  joystick_y);            
            sendJoystickUpdate(false);
        }
    };
     ...
     */
    private final IRuntimeInputPort ipJoystickR
            = new DefaultRuntimeInputPort()
    {
        public void receiveData(byte[] data)
        {
     ...
     */
    private final IRuntimeInputPort ipJoystickPov
            = new DefaultRuntimeInputPort()
    {
        public void receiveData(byte[] data)
        {
   ...
   */
   ...
   public void start()
   {
	   ...
	   if (port != null)
		  AstericsErrorHandling.instance.reportInfo(this, "RemoteJoystick Instance (ID "+propUniqueID+") started.");
	   else
      		AstericsErrorHandling.instance.reportError(this, "Could not find RemoteJoystick. Please verify that the HID actuator USB dongle is connected to the remote system and correctly installed on this computer.");
	   super.start();  
  }
