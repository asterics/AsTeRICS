 ...
 * 
 * Have a look at "http://www.edfplus.info/specs/edf.html" for full file
 * specification
 * 
 * @author Christian Strasak [christian.strasak@technikum-wien.at] Date:
 *         15.07.2014
 */
public class WriteEDFInstance extends AbstractRuntimeComponentInstance implements Runnable {
    ...
    int signalOfInputChannel[] = new int[NUMBER_OF_CHANNELS]; // for assigning
                                                              // the channels
                                                              // IDs to signal
                                                              // IDs
    ...
    ByteArrayOutputStream[][] samplesOfSignal = new ByteArrayOutputStream[NUMBER_OF_CHANNELS][NUMBER_OF_STREAM_BUFFERS]; // streams
                                                                                                                         // for
                                                                                                                         // the
                                                                                                                         // samples
                                                                                                                         // data
    ...
    /**
     * converts parameter doubleData into bytes and appends it to the stream of
     * parameter signalNumber
     * 
     * @author Christian Strasak
     * @param doubleData
     *            value, which should be appended to a stream
     * @param signalNumber
     *            number of the signal stream, in which doubleData should be
     *            appended
     * @return void
     */
    public void appendSampleToStream(double doubleData, int signalNumber) {
        int dataInt = (int) ((doubleData - physicalMinimumOfSignal[signalNumber])
                / (physicalMaximumOfSignal[signalNumber] - physicalMinimumOfSignal[signalNumber])
                * (digitalMaximumOfSignal[signalNumber] - digitalMinimumOfSignal[signalNumber])
                + digitalMinimumOfSignal[signalNumber]);
    ...
    /**
     * writes parameter data into a stream, manages the stream buffers and
     * deactivates a signal, if a buffer-overflow occurs
     * 
     * @author Christian Strasak
     * @param data
     *            value which should be stored
     * @param signalNumber
     *            number of the signal stream, in which data should be appended
     * @return void
     */
    public void storeValueIntoStreams(byte[] data, int signalNumber) {
        if (!samplingShouldPause && signalIsActivated[signalNumber]) {
            ...
            if ((samplesOfSignal[signalNumber][streamBufferWritePointer[signalNumber]].size()
                    / 2) >= samplingRateOfSignal[signalNumber]) {
                // ===================================================================================================
                // incrementing the stream buffer, if the current is full
                // the buffer is realized as a ring buffer, so it will be set to
                // zero after each cycle
                // ===================================================================================================
                ...
                // if it is not empty, the file storage of this buffer was not
                // completed yet
                ...
                if (samplesOfSignal[signalNumber][streamBufferWritePointer[signalNumber]].size() != 0) {
                    AstericsErrorHandling.instance.getLogger().warning("writeData: signal " + signalNumber
                            + " has been deactivated, because the buffer has been overflowed");
                    ...
                    // ===================================================================================================
                    for (int streamBufferNumber = 0; streamBufferNumber < NUMBER_OF_STREAM_BUFFERS; streamBufferNumber++) {
                        samplesOfSignal[signalNumber][streamBufferNumber].reset(); // reset
                                                                                   // all
                                                                                   // stream
                                                                                   // buffers
                                                                                   // for
                                                                                   // this
                                                                                   // signal
                        for (int sampleNumber = 0; sampleNumber < (samplingRateOfSignal[signalNumber]
                                * 2); sampleNumber++) {
                            samplesOfSignal[signalNumber][streamBufferNumber].write(0); // fill
                                                                                        // each
                                                                                        // stream
                                                                                        // with
                                                                                        // zeros
    ...
    public void start() {
        ...
        // ===================================================================================================
        String formatVersion = appendSpaceToString("0", 8, false); // 0
                                                                   // indicates
                                                                   // that it is
                                                                   // written in
                                                                   // EDF-format
        ...
        String headerBytes = appendSpaceToString(String.valueOf(256 * (1 + numberOfSignals)), 8, false);// Size
                                                                                                        // of
                                                                                                        // header
                                                                                                        // in
                                                                                                        // bytes
        ...
        // ===================================================================================================
        String label = appendSpaceToString("label", 16, true); // (e.g. EEG
                                                               // Fpz-Cz or Body
                                                               // temp)
        String transducerType = appendSpaceToString("transducerType", 80, true); // (e.g.
                                                                                 // AgAgCl
                                                                                 // electrode)
        String physicalDimension = appendSpaceToString("units", 8, true); // (e.g.
                                                                          // uV
                                                                          // or
                                                                          // degreeC)
        ...
        String prefiltering = appendSpaceToString("preFiltering", 80, true); // (e.g.
                                                                             // HP:0.1Hz
                                                                             // LP:75Hz)
        ...
        String reserved2 = appendSpaceToString("", 32, true); // reserved due to
                                                              // EDF
                                                              // specification
        // ===================================================================================================
        ...
        try {
            ...
            AstericsErrorHandling.instance.getLogger().severe("Error: could not create subfolders");
        }

        BufferedWriter out = null;
        try {
            out = new BufferedWriter(new FileWriter(filePathNameExtension));
            out.write(formatVersion + "" + patientID + recordingID + startDate + startTime + headerBytes + reserved
                    + numberOfRecords + durationOfRecords + signals + label + transducerType + physicalDimension
                    + physicalMinimum + physicalMaximum + digitalMinimum + digitalMaximum + prefiltering
                    + numberOfSamples + reserved2);
        } catch (IOException e) {
            AstericsErrorHandling.instance.getLogger().severe("Error writing header to file");
        }
        ...
        // ===================================================================================================
        recordingShouldRun = true; // sends the recording threads the start
                                   // signal
        ...
        while (!recordingRuns) // waits until the threads are started before
                               // calling the "super.start()"
        {
            try {
    ...
    /**
     * This class checks every record time, if the buffers for the signals are
     * full if full: the store method will be called if not full: the
     * corresponding signal will be deactivated
     **/
    public class WriteStreamsToFile extends Thread {
        ...
        public void run() {
            ...
            while (recordingShouldRun) {
                ...
                // ===================================================================================================
                // Waiting until the next record time (when the stream buffers
                // should be full) + recording time buffer
                // ===================================================================================================
                ...
                if (timeUntilNextStorage > 0) {
                    try {
                        TimeUnit.NANOSECONDS.sleep(timeUntilNextStorage);
                    } catch (Exception e) {
                        AstericsErrorHandling.instance.getLogger().warning("could not sleep until next record");
                    }
                    ;
                } else {
                    AstericsErrorHandling.instance.getLogger()
                            .warning("time until next record: " + timeUntilNextStorage);
                }

                // AstericsErrorHandling.instance.getLogger().info("read buffer
                // pointer: " + streamBufferReadPointer);

                for (int signalNumber = 0; signalNumber < numberOfSignals; signalNumber++) {

                    // AstericsErrorHandling.instance.getLogger().info("write
                    // buffer pointer of signal " + signalNumber + ": " +
                    // streamBufferWritePointer[signalNumber]);
                    // ===================================================================================================
                    // checking if the buffers of the current cycle of all
                    // signals are already full
                    // if not, than the corresponding signal will be deactivated
                    // and all stream buffers of this signal will be reseted and
                    // filled with zeors
                    // this could be caused by a too fast frequency property
                    // ===================================================================================================
                    if (signalIsActivated[signalNumber]
                            && (samplesOfSignal[signalNumber][streamBufferReadPointer].size()
                                    / 2) < samplingRateOfSignal[signalNumber]) {
                        signalIsActivated[signalNumber] = false;

                        if (samplesOfSignal[signalNumber][streamBufferReadPointer].size() == 0) {
                            AstericsErrorHandling.instance.getLogger().warning("signal " + signalNumber
                                    + " has been deactivated, because the buffer was empty at the storage timepoint");
                        } else {
                            AstericsErrorHandling.instance.getLogger().warning("signal " + signalNumber
                                    + " has been deactivated, because the buffer was not full at the storage timepoint");
                        }
    ...
    /**
     * thread for the reSampling method this method calls threads for every
     * signal, which will handle the incoming data of the input ports the called
     * threads will run for the defined record time and then quit it waits until
     * all called threads have quit and restarts them until the stop button has
     * been pressed
     */
    @Override
    public void run() {
    ...
    /**
     * this class reads the buffer of a input port periodically (defined by the
     * sampling rate) after the defined record time it calls the store method
     * and quits {@value signalNumberParameter number of the signal, which will
     * be stored}
     */
    public class BufferSampling extends Thread {
         ...
         * Constructor
         * 
         * @param signalNumberParameter
         *            = number of the signal to process
         */
        ...
        public void run() {
            ...
            long periodDuration = (samplingRateOfSignal[signalNumber] < 1) ? 0
                    : (1000 * 1000 * 1000 / samplingRateOfSignal[signalNumber]);
            ...
            for (int sampleNumber = 1; sampleNumber <= samplingRateOfSignal[signalNumber]; sampleNumber++) {
                ...
                if (timeUntilNextSample > 0) {
                    try {
                        TimeUnit.NANOSECONDS.sleep(timeUntilNextSample);
                    } catch (InterruptedException e) {
                    }
                    ;
                }
                ...
                if (samplingShouldPause) {
                    ...
                    // ===================================================================================================
                    // Correcting the time and sampling offset after the pause
    ...
    /**
     * this method appends the values of the current stream buffer pointer, of
     * all signals, to the file
     * 
     * @param bufferPointer
     *            pointer to the stream buffers
     */
    public void writeDataStreamsToFile(int bufferPointer) {
        ...
        try {
            ...
            for (int signalNumber = 0; signalNumber < numberOfSignals; signalNumber++) {
                // ===================================================================================================
                // Checking if the buffers of the current read-pointer of each
                // signal are full
            ...
            AstericsErrorHandling.instance.getLogger().severe("Error appending data streams to file");
        }
        // ===================================================================================================
        ...
        try {
            ...
            file.seek(236); // go to position of "number of data records" in
                            // file
            file.writeBytes(appendSpaceToString(String.valueOf(++numberOfDataRecords), 8, false));
            file.close();
        } catch (IOException e) {
            AstericsErrorHandling.instance.getLogger()
                    .info("Error writing value for number of data records in file: " + propFileName);
        }
    }

    /**
     * this method appends several " " (spaces) to [value], so they can be
     * stored in the correct length into the header of an edf file
     * 
     * @param value
     *            value to which the spaces will be appended
     * @param length
     *            the correct length (defined by the EDF specification)
     * @param multi
     *            if true: this will be done [numberOfSignals] times, else: only
     *            once
    ...
    public String appendSpaceToString(String value, final int length, boolean multi) {
        ...
        for (int signalNumber = 0; signalNumber < signals; signalNumber++) {
            if (value.length() > length) {
                AstericsErrorHandling.instance.getLogger().warning("value " + value
                        + " will be cut on the right side, because it is too big to write it in the file");
            }

            try {
                newString += value.substring(0, length);
