...
import static eu.asterics.ape.main.APEProperties.DEFAULT_PROJECT_DIR;
import static eu.asterics.ape.main.APEProperties.P_APE_BUILD_DIR;
import static eu.asterics.ape.main.APEProperties.P_APE_LOG_LEVEL;
import static eu.asterics.ape.main.APEProperties.P_APE_MODELS;
import static eu.asterics.ape.main.APEProperties.P_APE_PROJECT_DIR;
import static eu.asterics.ape.main.APEProperties.P_ARE_BASE_URI;
...
/**
 * This is the main class for the APE tool. Main tasks are configuring the tool
 * with the set properties and starting the copying process. Author:
 * martin.deinhofer@technikum-wien.at Date: Oct 30, 2015 Time: 14:30:00 PM
 */
...
public class APE {
    ...
    public static void main(String[] args) throws IOException, ParseException, URISyntaxException,
        ...
        // Parse the arguments, check if --help is requested or otherwise
        // -Darguments are given that must be treated as property settings
        for (String arg : args) {
    ...
    /**
     * Initilizes APE.baseURI with a 3-way fallback mechanism. 1) Use current
     * working directory (CWD) 2) Use location of APE.jar 3) Use value of
     * property APE.baseURI set as system property (-DAPE.baseURI=...)
     * 
     * @return
     */
    private static URI initAPEBaseURI() {
        ...
        Notifier.debug(
                "SysProp[" + APEProperties.P_APE_BASE_URI + "]=" + System.getProperty(APEProperties.P_APE_BASE_URI),
                null);
        String newApeBaseURIString = System.getProperty(APEProperties.P_APE_BASE_URI);
        if (newApeBaseURIString != null) {
            // Resolve it against the CWD because this is the only intuitive
            // approach if the user provides a new APE.baseURI via commandline
     ...
     */
    public void start() throws IOException, ParseException, URISyntaxException, ParserConfigurationException,
    ...
    /**
     * Determins property file location (APE.properties), reads property values
     * and overrides property values given as system property (-Dkey=value).
     * 
     * @throws IOException
     */
    private void initProperties() throws IOException {
        ...
        try {
            ...
            for (Entry<Object, Object> entry : System.getProperties().entrySet()) {
                if (entry.getKey().toString().startsWith(APEProperties.APE_PROP_PREFIX)
                    ...
                    if (entry.getKey().equals(P_APE_MODELS) || entry.getKey().equals(P_APE_BUILD_DIR)
                        ...
                        // if the user provided model paths via the system
                        // property and not via the properties file, we have to
                        // resolve relative paths to the current directory
                        // and not relative to the APE.projectDir location to
            ...
            // Also add the projectDir to the properties to be sure that the
            // values are in sync.
    ...
    /**
     * Resolve the given String with relative model paths to strings with
     * absolute model paths resolved against the CWD.
     * 
     * @param relativePaths
     * @return
     */
    ...
    /**
     * Sets APE.baseURI to the given value. APE.baseURI will be used to fetch
     * template data and lookup for the properties file APE.properties by
     * default.
     * 
     * @param APEBaseURI
     */
    ...
    /**
     * Returns a reference to the Packager that actually does all the copying of
     * resources.
     * 
     * @return
     */
