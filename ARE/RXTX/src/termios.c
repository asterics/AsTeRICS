...
int serial_test( char * filename )
{
    ...
    if (bGetVer && (osvi.dwPlatformId == VER_PLATFORM_WIN32_NT))
    {
    	...
    	dwChars = QueryDosDevice(NULL, szDevices, 65535);
        if (dwChars)
        {
          int i=0;
	...
	unsigned long *hcomm;
	int ret;
	printf("Attempting on port %s\n", filename);
	hcomm = CreateFile( filename, GENERIC_READ |GENERIC_WRITE, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OVERLAPPED, 0 );
	if ( hcomm == INVALID_HANDLE_VALUE )
	{
		DWORD error = GetLastError();
		if ((error == ERROR_ACCESS_DENIED) || (error == ERROR_SEM_TIMEOUT))
		{
			ret = 1;
		}
		else
		{
			printf("Error on port %s: %d\n", filename, error);
			ret = 0;
		}
	}
	else
...
*/
	...
	if ( !WriteFile( index->hComm, Str, length, &nBytes, &index->wol ) )
	{
					...
					&nBytes, FALSE ) )
		{
			...
			if (timeout_cnt > 100)
			{
				report("\"timeout\" on getoverlapped result\n");
				nBytes = -1;
				goto end;
			}
			timeout_cnt++;
...
void termios_interrupt_event_loop( int fd, int flag )
{
	...
	if ( !index )
	{
		report( "termios_interrupt_event_loop !index" );
		return;
	}
/*
	index->event_flag = 0;
...
*/
	...
	report( "termios_interrupt_event_loop" );
	return;
}
