 ...
 *         Time: 02:22:08 PM
 */
class CIMSerialPortController extends CIMPortController implements 
Runnable
{
	private final int PACKET_TIMEOUT = 3000;
	private final int PACKET_DETECT_SEND_OVERHEAD = 1000;
	private final int PACKET_DETECT_TIMEOUT = PACKET_TIMEOUT + PACKET_DETECT_SEND_OVERHEAD;

	private boolean hasIdentifiedConnectedCIM = false;
	 ...
	 */
	CIMSerialPortController(String comPortName, SerialPort port, 
			CIMPortEventListener listener)
	{
		super(comPortName);
		this.port = port;
		...
		while (threadRunning)
		{
			try {
					...
					case PACKET_FOUND:
						synchronized (this)
						{
							if (packet.parsePacket(b))
							{
								...
								if (errorCode != CIMProtocolPacket.ERRORSTATE_DROP_PACKET && errorCode != CIMProtocolPacket.ERRORSTATE_NOT_READY )
								{
									...
									if ((receivedSerialNumber & 0x80) != 0)
									{
										...
										{
											//												Logger.getAnonymousLogger().warning("Did not receive correct packet serial number on CIM generated packet, potentially lost packets, " + packet.toString());
											logger.warning(this.getClass().getName()+".run: Did not receive correct packet serial number on CIM generated packet, potentially lost packets, " + packet.toString());
											nextExpectedCIMIssuedSerialNumber = receivedSerialNumber;
										}
										...
										synchronized (eventHandlers)
										{
											...
											{
												eventHandler.handlePacketReceived(new CIMEventPacketReceived(this, packet));
											}
										}
									}
									else
									{
										// this is a reply to an ARE generated packet
										if (nextExpectedIncomingSerialNumber != receivedSerialNumber)
										{
											// some packets have been lost
											//												Logger.getAnonymousLogger().severe("Lost one or more packets: expected: #" + nextExpectedIncomingSerialNumber + ", received: #" +  receivedSerialNumber);
											logger.severe(this.getClass().getName()+".run: Lost one or more packets: expected: #" + nextExpectedIncomingSerialNumber + ", received: #" +  receivedSerialNumber);
											if (!eventHandlers.isEmpty())
											{
												// notify the caller about all missing packets
												while (nextExpectedIncomingSerialNumber != receivedSerialNumber )
												{
													synchronized (eventHandlers)
													{
														for (CIMEventHandler eventHandler : eventHandlers)
														{
															eventHandler.handlePacketError(new CIMEventErrorPacketLost(this,nextExpectedIncomingSerialNumber));
														}
													}
											...
											{
												// no need to notify just skip the missing numbers
								...
								else  // received with errors
								{
									...
									//									Logger.getAnonymousLogger().severe("Error in packet #" + packet.getSerialNumber() + "on port " + comPortName);
									logger.severe(this.getClass().getName()+".run: Error in packet #" + packet.getSerialNumber() + "on port " + comPortName);
				...
				else
				{
					...
					if (lastPacketSent != 0)
					{
						if  ((nextExpectedIncomingSerialNumber != serialNumber) &&  (lastPacketSent + PACKET_TIMEOUT < System.currentTimeMillis()))
						{
							synchronized (eventHandlers)
							{
								for (CIMEventHandler eventHandler : eventHandlers)
							...
							logger.warning(this.getClass().getName()+
									".run: Did not receive reply to packet #" 
									+ nextExpectedIncomingSerialNumber + 
									" since " + PACKET_TIMEOUT + "ms on " +
									 comPortName);
							nextExpectedIncomingSerialNumber =(byte) ((nextExpectedIncomingSerialNumber + 1 ) % 128);
							lastPacketSent = 0;
						}
	...
	/* (non-Javadoc)
	 * @see eu.asterics.mw.cimcommunication.CIMPortController#sendPacket(byte[], short, short, boolean)
	 */
	@Override
...
//				}
	...
	public void setNextExpectedIncomingSerialNumber(
			byte nextExpectedIncomingSerialNumber) 
	{
