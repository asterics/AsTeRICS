...
public class CIMProtocolPacket {
    ...
    /**
     * Constructor that creates a CIMProtocolPacket from an input stream. This
     * constructor is used to read in packets from the serial port controller.
     * 
     * @param in
     * @param crcOn
    ...
    public CIMProtocolPacket(InputStream in, boolean crcOn) throws CIMProtocolException {
         ...
         * checkForPacketBoundary and the next byte will be the LSB of software
         * version
         */
        try {
    ...
    long leParseLong(byte[] data) {
        return (long) (((data[3] << 24) & 0xff000000) | ((data[2] << 16) & 0x00ff0000) | ((data[1] << 8) & 0x0000ff00)
                | (data[0] & 0x000000ff));
    }
     ...
     * 
     * @param b
     *            the next byte in the packet/stream
    ...
    public String toString() {
        // TODO finish this
        StringBuffer buf = new StringBuffer();
        buf.append("AreCimID: ").append(String.format("0x%x", this.areCimID)).append(", serialNumber: ")
                .append(this.serialNumber).append(", featureAddress: ")
                .append(String.format("0x%x", this.featureAddress)).append(",").append(requestReplyCodeToString())
                .append(", crc:").append(this.crcOn ? "on" : "off");

        if (data != null) {
            buf.append(", data lg: ").append(data != null ? data.length : 0).append(", data: ");
    ...
    public int receivedWithoutErrors() {
        ...
        if ((errorWord & STATUS_ERROR_MASK) != 0) {
            if (!checkCrc()) {
                logger.warning(this.getClass().getName() + "." + "receivedWithoutErrors:"
            ...
            if ((errorWord & STATUS_ERROR_LOST_PACKET_MASK) != 0) {
                if ((requestReplyCode & 0xff) != 0x80) {
                    logger.warning(this.getClass().getName() + "." + "receivedWithoutErrors: Faulty packet received: "
