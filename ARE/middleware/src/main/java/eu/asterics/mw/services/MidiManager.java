...
public class MidiManager {
    ...
    public static MidiManager instance = new MidiManager();
    ...
    private MidiManager() {
        if (debugOutput) {
            System.out.println("MidiManger Started");
        }
        if (midiDeviceInfos == null) {
            if (debugOutput) {
                System.out.println("MidiManger: first start - getting  Midi Deviceinfos!");
            }
    ...
    /**
     * Returns a vector of all playable Midi Devices (Synthesizers or
     * Transmitters)
     * 
     * @return a vector of all playable Midi Devices (Synthesizers or
     *         Transmitters)
     */
    public Vector<Info> getActDeviceInfos() {
        Vector<Info> actDeviceInfos = new Vector<Info>(); // Vector<Info> =
                                                          // Generic
        MidiDevice.Info[] deviceInfo = MidiSystem.getMidiDeviceInfo(); // gets a
                                                                       // String
                                                                       // of
                                                                       // available
                                                                       // MIDI-devices.
        for (int i = 0; i < deviceInfo.length; i++) {
            try {
                ...
                if ((device instanceof Synthesizer) || (device.getMaxTransmitters() > -1)) {
                    if (debugOutput) {
                        System.out.println("MidiManager: Playable device found: " + device.getDeviceInfo().getName());
                    }
     ...
     * 
     * @param deviceInfo
     *            information of type Info about the MidiDevice
    ...
    public boolean openMidiDevice(String newDeviceName) // MidiDevice.Info
                                                        // deviceInfo)
    {
        if (debugOutput) {
            System.out.println("Midimanager: open called.");
        }
        if (midiUnitsByName.get(newDeviceName) != null) {
            if (debugOutput) {
                System.out.println("Midimanager: Device " + newDeviceName + " found in map returning !");
            }
            return true;
        }
        ...
        if (debugOutput) {
            System.out.println(
                    "Midimanager: Device " + newDeviceName + " not fount in map, trying to add / open the device.");
        }
        ...
        if (debugOutput) {
            System.out.println("Midimanager: " + deviceInfo.getName() + " found - trying to open.");
        }
        try {
            ...
            if (debugOutput) {
                System.out.println("GetMididevice.");
            }
            ...
            if (midiDevice instanceof Synthesizer) {
                ...
                if (synthesizer.isOpen() == false) {
                    if (debugOutput) {
                        System.out.println("Open Synth.");
                    }
                    synthesizer.open();
                    if (debugOutput) {
                        System.out.println(
                                "Midimanager: Synthesizer " + midiDevice.getDeviceInfo().getName() + " opened.");
                    }
                } else if (debugOutput) {
                    System.out
                            .println("Midimanager: Synthesizer " + midiDevice.getDeviceInfo().getName() + " reopened.");
                }

                if (debugOutput) {
                    System.out.println("Generate MIDI unit.");
                }
                ...
                if (debugOutput) {
                    System.out.println("Get Instruments.");
                }
                ...
                if (midiDevice.isOpen() == false) {
                    if (debugOutput) {
                        System.out.println("Open Midi Device.");
                    }

                    midiDevice.open();
                    if (debugOutput) {
                        System.out.println(
                                "Midimanager: MidiDevice " + midiDevice.getDeviceInfo().getName() + " opened.");
                    }
                } else {
                    if (debugOutput) {
                        System.out.println(
                                "Midimanager: MidiDevice " + midiDevice.getDeviceInfo().getName() + " reopened.");
                    }
                }
                receiver = midiDevice.getReceiver(); // MidiSystem.getReceiver();

                if (debugOutput) {
                    System.out.println("Open Midi Unit.");
                }
    ...
    public void instrumentChange(String deviceName, int channel, String newInstrumentName) {
        ...
        if (midiUnit.synthesizer != null) {
            if (debugOutput) {
                System.out.println("MidiManager: changing Synth channel " + channel + " to instrument " + instrument);
            }
            midiUnit.midiChannels[channel].programChange(instrument);
        } else if ((midiUnit.receiver != null) && (instrument < 128)) {
            if (debugOutput) {
                System.out.println(
                        "MidiManager: changing Mididevice channel " + channel + " to instrument " + instrument);
            }
    ...
    public void midiAllSoundOff(String deviceName, int channel) {
        ...
        if (midiUnit.synthesizer != null) {
            midiUnit.midiChannels[channel].allSoundOff();
        }
    }
    ...
    public void midiNoteOn(String deviceName, int channel, int note, int velocity) {
        ...
        if (midiUnit.synthesizer != null) {
            if (debugOutput) {
                System.out.println(
                        "Midimanager: Channel  " + channel + " Note " + note + " on using synth, velocity " + velocity);
            }
            midiUnit.midiChannels[channel].noteOn(note, velocity);
        } else if (midiUnit.receiver != null) {
            try {
                if (debugOutput) {
                    System.out.println("Midimanager: Channel  " + channel + " Note " + note
                            + " on using Midi Unit, velocity " + velocity);
                }
                ShortMessage myMsg = new ShortMessage();
    ...
    public void midiNoteOff(String deviceName, int channel, int note) {
        ...
        if (midiUnit.synthesizer != null) {
            if (debugOutput) {
                System.out.println("Channel  " + channel + " Note " + note + " off using synth");
            }
        ...
        if (midiUnit.receiver != null) {
            try {
                if (debugOutput) {
                    System.out.println("Channel  " + channel + " Note " + note + " off using midi unit");
                }
                ShortMessage myMsg = new ShortMessage();
    ...
    public String[] getGeneralMidiInstrumentNames() {
        String[] names = { "1 Acoustic Grand Piano", "2 Bright Acoustic Piano", "3 Electric Grand Piano",
                "4 Honky-tonk Piano", "5 Electric Piano 1", "6 Electric Piano 2", "7 Harpsichord", "8 Clavinet",
                "9 Celesta", "10 Glockenspiel", "11 Music Box", "12 Vibraphone", "13 Marimba", "14 Xylophone",
                "15 Tubular Bells", "16 Dulcimer", "17 Drawbar Organ", "18 Percussive Organ", "19 Rock Organ",
                "20 Church Organ", "21 Reed Organ", "22 Accordion", "23 Harmonica", "24 Tango Accordion",
                ...
                "46 Pizzicato Strings", "47 Orchestral Harp", "48 Timpani", "49 String Ensemble 1",
                "50 String Ensemble 2", "51 Synth Strings 1", "52 Synth Strings 2", "53 Choir Aahs", "54 Voice Oohs",
                "55 Synth Voice", "56 Orchestra Hit", "57 Trumpet", "58 Trombone", "59 Tuba", "60 Muted Trumpet",
                "61 French Horn", "62 Brass Section", "63 Synth Brass 1", "64 Synth Brass 2", "65 Soprano Sax",
                "66 Alto Sax", "67 Tenor Sax", "68 Baritone Sax", "69 Oboe", "70 English Horn", "71 Bassoon",
                "72 Clarinet", "73 Piccolo", "74 Flute", "75 Recorder", "76 Pan Flute", "77 Blown Bottle",
                "78 Shakuhachi", "79 Whistle", "80 Ocarina", "81 Lead 1 (square)", "82 Lead 2 (sawtooth)",
                "83 Lead 3 (calliope)", "84 Lead 4 (chiff)", "85 Lead 5 (charang)", "86 Lead 6 (voice)",
                "87 Lead 7 (fifths)", "88 Lead 8 (bass + lead)", "89 Pad 1 (new age)", "90 Pad 2 (warm)",
                "91 Pad 3 (polysynth)", "92 Pad 4 (choir)", "93 Pad 5 (bowed)", "94 Pad 6 (metallic)",
                "95 Pad 7 (halo)", "96 Pad 8 (sweep)", "97 FX 1 (rain)", "98 FX 2 (soundtrack)", "99 FX 3 (crystal)",
                "100 FX 4 (atmosphere)", "101 FX 5 (brightness)", "102 FX 6 (goblins)", "103 FX 7 (echoes)",
                "104 FX 8 (sci-fi)", "105 Sitar", "106 Banjo", "107 Shamisen", "108 Koto", "109 Kalimba",
                "110 Bag pipe", "111 Fiddle", "112 Shanai", "113 Tinkle Bell", "114 Agogo", "115 Steel Drums",
                "116 Woodblock", "117 Taiko Drum", "118 Melodic Tom", "119 Synth Drum", "120 Reverse Cymbal",
                "121 Guitar Fret Noise", "122 Breath Noise", "123 Seashore", "124 Bird Tweet", "125 Telephone Ring",
