 ...
 * 
 */
public abstract class AbstractRuntimeEventTriggererPort implements IRuntimeEventTriggererPort {
    ...
    public void raiseEvent() {
        AstericsModelExecutionThreadPool.instance.execute(new Runnable() {
            ...
            public void run() {
                ...
                // syncing on eventListeners ensures that a deployed model is
                // not changed during event notification.
                // logger.fine("Synchronizing on eventListeners, curThread:
                // "+Thread.currentThread().getName()+", eventListeners:
                ...
                // synchronized (eventListeners)
                {
                    for (Map.Entry<String, IRuntimeEventListenerPort> elem : eventListeners.entrySet()) {
                        ...
                        // block event propagation if model and component is not
                        // up and running
                        // Disabled for now, because some important events are
                        // lost at startup.
                        /*
                         * if(!DeploymentManager.instance.isComponentRunning(
                         * targetComponentId)) { //logger.warning(
                         * "Event could not be notified, target component not running: targetComponentId: "
                         * +targetComponentId); //System.out.println("E: "
                         * +targetComponentId); //return; }
                         */
                        ...
                        if (targetComponent != null) {
                            ...
                            // synchronized (targetComponent)
                            {
                                ...
                                RuntimeDataEvent event = RuntimeDataEvent.newEventChannelTransmission(channelID,
                                        targetComponentId);
                                AREServices.instance.notifyRuntimeDataListeners(event);
                            }
                        } else {
                            logger.warning(
                                    "Event could not be notified, target component not found: targetComponentId: "
                                            + targetComponentId);
                        }
                    }
                }
    ...
    public void addEventListener(String targetComponentID, String eventPortID,
        ...
        // MULTI-THREADED: Remove comments if you want to reenable
        // multi-threaded execution approach.
        // syncing on eventListeners ensures that a deployed model is not
        // changed during event notification.
        // synchronized (eventListeners)
        {
