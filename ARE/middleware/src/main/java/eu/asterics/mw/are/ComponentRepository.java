...
public class ComponentRepository {
    public static final ComponentRepository instance = new ComponentRepository();
    ...
    private final Map<String, DefaultComponentFactory> componentFactories = new HashMap<String, DefaultComponentFactory>();
...

    {
        ...
        // System.out.println ("Installing "+componentTypeID+" -
        // "+componentType);
    }

    void uninstall(final IComponentType componentType) throws BundleManagementException {
        if (componentType == null) {
            logger.severe(this.getClass().getName() + ": uninstall-> Illegal null argument");
            throw new NullPointerException("Illegal null argument");
        }
    ...
    void uninstall(final String componentTypeID, String componentCanonicalName) throws BundleManagementException {
        if (!repository.containsKey(componentTypeID)) {
            logger.severe(": uninstall-> The specified componentTypeID"
                    + " is not installed in the repository, can't uninstall it.");
            throw new BundleManagementException("The specified componentTypeID" + " is not found in the repository.");
        }
    ...
    public IComponentType getComponentType(final String componentTypeID) throws BundleManagementException {
        if (!repository.containsKey(componentTypeID)) {
            // System.out.println("*** Requesting installation of component "
            // +cTypeID);
    ...
    public void setComponentFactory(String componentCanonicalName, DefaultComponentFactory defaultComponentFactory) {
        this.componentFactories.put(componentCanonicalName, defaultComponentFactory);
