						...
						if (runtimeInputPort.isBuffered()) {
							...
							//synchronized(targetComponent) 
							{
								...
								try {
									...
									Set<IChannel> channels = DeploymentManager.instance.getCurrentRuntimeModel().getChannels();
									...
									//Convert the data to the actual type and save them as sctring characters
									DataType portDataType = DeploymentManager.instance.getCurrentRuntimeModel().getPortDataType(endPoint.targetComponentID, endPoint.portID);
									String listenerData = "";
									switch (portDataType) {
										case BOOLEAN:
									...
									if (!currentChannelId.isEmpty()) {
		    							RuntimeDataEvent event = RuntimeDataEvent.newDataChannelTransmission(currentChannelId, listenerData);
		    							AREServices.instance.notifyRuntimeDataListeners(event);
									}
								} catch (Exception ex) {
									//error while trying to transmit channel data to a remote subscriber
								}
