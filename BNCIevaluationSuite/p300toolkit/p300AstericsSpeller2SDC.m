function data=p300AstericsSpeller2SDC(OUTEEG,block)

%this function takes BDF data format as open by pop_biosig function (OUTEEG), applies some filters and
...
%data.signal: 3D matrix (32*3073*810) 32 channel*epoch length*number of epochs

%data.GT: 810*3 matrix. 
...
% p300AstericsSpeller2SDC(OUTEEG,block)
...
% %absoluteRoute
% absoluteRoute=['/subject',num2str(subject),'/session',num2str(session),'/s',num2str(subject),'b',num2str(block),'sp.bdf'];
%%%%%%%%%%%changed for windows accessing data @ skat (taken out in order to simplify
%%%%%%%%%%%function)
% localRoute=['Z:\S1-ONGOING-PROJECTS\ASTERICS\asterics\'];
% %absoluteRoute
% absoluteRoute=['subject',num2str(subject),'\session',num2str(session),'\s',num2str(subject),'b',num2str(block),'sp.bdf'];
...
SDC=zeros(32,3073,810);%to build StarlabDataCube format
close all
for k=1:9 %symbol loop
disp(['Symbol code ',imagesNames{k}])
% figure
% set(k,'name',imagesNames{symbolCode(k)})
SDC1=[];%to build StarlabDataCube format
    for j=1:32 %channel loop
            attendedEpoch=[];
            for i=1:length(attendedStimuliLat{k})                               
                %I cut 0.5 sec (1024 samples) before the stimuli onset and 1 sec after
...
%         title(tit)
    ...
    %building StarlabDataCube format
        for epoch=1:90
            ...
            SDC(j,:,1+90*(k-1)+(epoch-1))=SDC1(1+90*(j-1)+(epoch-1),:);
        end
    end %end channel loop (j)
    clear SDC1;    
end %end symbol loop
